...
.. figure:: _static/conflict_monitoring_pnl.svg
   :figwidth: 45 %
   :align: left
...
Network System
~~~~~~~~~~~~~~
**COLOR INPUT LAYER**:  a `ProcessingMechanism` with **input_shapes** = 3 (one unit for the input of one color, respectively
here blue & green), and assigned a `Linear` function with **slope** = 1.0 and **intercept** = 0.0.

**WORD INPUT LAYER**:  a `ProcessingMechanism` with **input_shapes** = 3 (one unit for the input of one word, respectively,
here blue & green), and assigned a `Linear` function with **slope** = 1.0 and **intercept** = 0.0.

**TASK INPUT LAYER**:  a `ProcessingMechanism` with **input_shapes** = 2 (one unit specified with a task
...
element for the neutral color and assigned a `Logistic` function with **gain** = 1.0 and **bias** = 4.0.
The **integrator_mode** = `True` and **integration_rate** = 0.01. Both units receive mutually inhibitory weights
(**hetero** = -2).
...
and **integration_rate** = 0.01. Both units receive mutually inhibitory weights (**hetero** = -2).
...
period of 500 time steps. Then the stimulus is presented for 600 time steps and is presented by setting the input
units to 1.0 for a given trial. Conflict is computed on the `output_port` of the *RESPONSE LAYER*. The figure plots
conflict over one trial for each of the three conditions starting after the settling period.
