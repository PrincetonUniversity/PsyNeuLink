...
What the group model may say about the parameters together
-----------------------------------------------------------
...
The two settings are not independent. ``covariance="full"`` requires ``curvature="full"``,
and a fit that asks for the first without the second is refused. The group covariance is
built from the participants' posterior covariances, so off-diagonals estimated from a
curvature that never measured any would come from the spread of the modes alone -- which
says how the point estimates happen to line up, not how the population varies.
