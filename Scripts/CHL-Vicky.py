    ...
    def function(self,
        ...
        THE FOLOWING NEEDS TO BE REPLACED WITH CONTRASTIVE HEBBIAN LEARNING RULE:
           ...
           *learning_rate* * :math:`a_ia_j` if :math:`i \\neq j`, else :math:`0`
           ...
           VZ - learning rule : a_iPlus * a_jPlus - a_iMinus * a_jMinus
        ...
        variable : VZ - 2 arrays, variable[0] of + phase activity values and variable[1] of - phase activity values
        ...
        weight change matrix : 2d np.array
            VZ - difference between matrices of pairwise products of elements of variable[0] and variable[1] (`variable <ContrastiveHebbian.variable>`) scaled by the
        ...
        # VZ - If learning_rate is a 1d array, multiply variable[0] and variable[1] by learning_rate
        if self.learning_rate_dim == 1:
            variable[0] = variable[0] * learning_rate 
            variable[1] = variable[1] * learning_rate

        # IMPLEMENTATION NOTE:  THE FOLLOWING NEEDS TO BE REPLACED BY THE CONTRASTIVE HEBBIAN LEARNING RULE:

        # VZ - Generate the column array for plus and minus activity variables
        colPlus = np.array(np.matrix(variable[0]).T)
        colMinus = np.array(np.matrix(variable[1]).T)

        # VZ - Calculate weight change matrices
        weight_change_matrixPlus = variable[0] * colPlus
        weight_change_matrixMinus = variable[1] * colMinus
        
        # Zero diagonals (i.e., don't allow correlation of a unit with itself to be included)
        weight_change_matrixPlus = weight_change_matrixPlus * (1-np.identity(len(variable[0])))
        weight_change_matrixMinus = weight_change_matrixMinus * (1-np.identity(len(variable[1])))

        # If learning_rate is scalar or 2d, multiply it by the weight change matrices
        if self.learning_rate_dim in {0, 2}:
            weight_change_matrixPlus = weight_change_matrixPlus * learning_rate
            weight_change_matrixMinus = weight_change_matrixMinus * learning_rate

        return np.subtract(weight_change_matrixPlus, weight_change_matrixMinus)
