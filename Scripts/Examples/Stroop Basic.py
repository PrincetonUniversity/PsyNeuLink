...
#  WEIGHTS
c_ih = [[2.2, -2.2], [-2.2, 2.2]]
w_ih = [[2.6, -2.6], [-2.6, 2.6]]
c_hr = [[1.3, -1.3], [-1.3, 1.3]]
w_hr = [[2.5, -2.5], [-2.5, 2.5]]
c_t = [[4.0, 4.0], [0, 0]]
h_t = [[0, 0], [4.0, 4.0]]
r_d = [1,-1]
...
# COMPOSITION
c = pnl.Composition(name='STROOP MODEL')
c.add_c_node(ci)
c.add_c_node(wi)
c.add_c_node(ch)
c.add_c_node(wh)
...
c.add_c_node(r)
c.add_c_node(d)
c.add_projection(sender=ci, receiver=ch, n)
...
c_t = pnl.MappingProjection(matrix=[[4.0, 4.0], [0, 0]], name='COLOR NAMING')
h_t = pnl.MappingProjection(matrix=[[0, 0], [4.0, 4.0]], name='WORD READING')
...
r_d = pnl.MappingProjection(matrix=[1,-1], name='OUTPUT TO DECISION')
