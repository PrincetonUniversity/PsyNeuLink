...
def control_function(outcome):
    ...
    If outcome signifies a MATCH:
       set  EM[store_prob]=1 (as prep for next trial)
       terminate trial
    If outcome signifies a NON-MATCH:
       set  EM[store_prob]=0 (as prep for another retrieval from EM without storage)
       continue trial

    Notes:
    - outcome is passed as 2d array with a single 1d entry: [1,0]=MATCH, [0,1]=NON-MATCH
    - return value is used:
        - by control Mechanism to set ControlSignal for EM[store_prob] (per above)
        - by terminate_trial(), which is used by Condition specified as termination_processing for comp.run(),
            to determine whether to end or continue trial

    """
    ffn_result = outcome[0]
    if ffn_result[1] > ffn_result[0]:
        return 1
    else:                   # NON-MATCH:
        return 0
    return None
...
comp.run(inputs=input_dict,
         ...
         termination_processing={TimeScale.TRIAL: Condition(terminate_trial)},
