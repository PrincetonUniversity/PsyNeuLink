...
from .config import defaults as params
...
def construct_model(
    ...
    context = pnl.TransferMechanism(
        ...
        integrator_function=pnl.DriftOnASphereIntegrator(
            dimension=params.CONTEXT_SIZE,
            initializer=[0, 1, 0, 0, 0, 0, 0],
            default_variable=[0, 0, 0, 0, 0, 0, 0],
            rate=params.STATE_INTEGRATION_RATE,  # How much of the input to integrate
            input_space='target',
            noise=[0, 0, 0, 0, 0, 0],
        )
    )
    ...
    ego_comp.add_projection(
        pnl.MappingProjection(context, em.nodes[_context_name + QUERY])
    )
    ego_comp.add_projection(
        pnl.MappingProjection(state_input, em.nodes[_state_name + QUERY])
    )
...
#     seq_baseline = 20
#
#     memory_capacity = seq_baseline * 3 + seq_baseline * 2
...
#     states_reval, rewards_reval = data.get_reward_revaluation_trials(num_seqs=seq_baseline)
#     times = data.get_time_sequence(num_trials=(len(states)+len(states_reval)))
#
#     inputs = {
#         _state_input: states,
#         _time_input: times[:len(states)],
...
#         _task_input: [0] * len(states_reval),
#     }
