...
def construct_model():
    ...
    context_temp_layer = pnl.ProcessingMechanism(
        name=context_tmp_name,
        input_shapes=params.CONTEXT_SIZE
    )

    context_layer = pnl.ProcessingMechanism(
        name=context_name,
        input_shapes=params.CONTEXT_SIZE,
        default_variable=[.01]*params.CONTEXT_SIZE
    )
    ...
    em = pnl.EMComposition(name='em',
                           ...
                           softmax_threshold=None,#params.SOFTMAX_THRESHOLD,
                           fields={
                               state_input_name: {
    ...
    # context -> em
    EGO_comp.add_projection(
        pnl.MappingProjection(context_temp_layer, em.nodes[context_name + QUERY]))
    # reward -> em
    EGO_comp.add_projection(
        pnl.MappingProjection(reward_input_layer, em.nodes[reward_input_name + VALUE]))

    # Inputs to Context Tmp -------------------------------------------------------------------------
    # state -> context_temp_layer
    norm_integration_rate = (params.STATE_INTEGRATION_RATE + params.OLD_CONTEXT_INTEGRATION_RATE)
    state_integration_rate_to_tmp_norm = params.STATE_INTEGRATION_RATE / norm_integration_rate
    old_context_integration_rate_to_tmp_norm = params.OLD_CONTEXT_INTEGRATION_RATE / norm_integration_rate

    EGO_comp.add_projection(
        pnl.MappingProjection(state_input_layer, context_temp_layer,
                              matrix=np.eye(params.CONTEXT_SIZE) * state_integration_rate_to_tmp_norm))

    # context -> context_temp_layer
    EGO_comp.add_projection(
        pnl.MappingProjection(context_layer, context_temp_layer,
                              matrix=np.eye(params.CONTEXT_SIZE) * old_context_integration_rate_to_tmp_norm))


    # Inputs to Context -------------------------------------------------------------------------

    context_retrieved_to_context = params.RETRIEVED_CONTEXT_INTEGRATION_RATE
    context_temp_to_context = 1 - context_retrieved_to_context
    # retrieved to context
    EGO_comp.add_projection(
        pnl.MappingProjection(em.nodes[context_name + RETRIEVED], context_layer,
                              matrix=np.eye(params.CONTEXT_SIZE) * context_retrieved_to_context))

    # context temp to context
    EGO_comp.add_projection(
        pnl.MappingProjection(context_temp_layer, context_layer,
                              matrix=np.eye(params.CONTEXT_SIZE) * context_temp_to_context))

    # EGO_comp.scheduler.add_condition(context_temp_layer, pnl.BeforeNodes(em))
    EGO_comp.scheduler.add_condition(context_temp_layer, pnl.BeforeNodes(state_input_layer))
    # EGO_comp.scheduler.add_condition(context_temp_layer, pnl.BeforeNodes(context_layer))
    # EGO_comp.scheduler.add_condition(em, pnl.BeforeNodes(context_temp_layer))
    # EGO_comp.scheduler.add_condition(state_input_layer, pnl.BeforeNodes(context_temp_layer))
    # EGO_comp.scheduler.add_condition(context_layer, pnl.BeforeNodes(context_temp_layer))
...
def main():
    ...
    for i in range(len(states)):
        _in = {
            state_input_layer: states[i],
            time_input_layer: times[i],
            reward_input_layer: rewards[i],
        }
        model.run(inputs=_in, animate=True)
        print(model.results)
