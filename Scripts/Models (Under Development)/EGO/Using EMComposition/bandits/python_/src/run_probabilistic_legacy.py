...
def run_model_choices(
    ...
    if memory_capacity == 'max':
        memory_capacity = n_base_trials * 3
    ...
    if time_drift_noise is not None:
        times = gen_trials.get_time_sequence_event(_n_base_trials, 3, noise=time_drift_noise)
    else:
        times = gen_trials.get_time_sequence_event(_n_base_trials, 3)
    ...
    # -----------------------------
    if second_id == 3:
        reward_prob, terminal_id = (rew_probs[5], 5) if np.random.rand() < 0.5 else (rew_probs[6], 6)
    else:  # second_id == 4
        reward_prob, terminal_id = (rew_probs[7], 7) if np.random.rand() < 0.5 else (rew_probs[8], 8)
    # -----------------------------
