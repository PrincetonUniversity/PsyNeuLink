...
def get_random_memories_normalized(n, d):
    _x = np.random.rand(n, d)
    return _x / np.linalg.norm(_x, axis=1, keepdims=True)

def get_random_memories(n, d):
...
def gen_memories(states: np.ndarray,
    ...
    state_memories = get_random_memories_normalized(memory_capacity, state_d) * memory_init
    context_memories = get_random_memories_normalized(memory_capacity, context_d) * memory_init
    time_memories = get_random_memories_normalized(memory_capacity, time_d) * memory_init
    reward_memories = get_random_memories(memory_capacity, reward_d) * memory_init
...
def sample_memory(memories,
                  query,
                  state_retrieval_weight: float = 0,
