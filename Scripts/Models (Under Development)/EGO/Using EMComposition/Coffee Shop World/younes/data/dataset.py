...
class CSWDatasetWithFirst(dataset.Dataset):
    """
    ...
    def __init__(self, n_samples_per_context, contexts_to_load, probs=[1, 1, 1]) -> None:
        ...
        xs = one_hot_encode(np.array(self.all_trials), 11)


        # add the first state (x is all zeros)
        first_trial = torch.zeros((1, 11), dtype=xs.dtype, device=xs.device)
        self.xs = torch.cat([first_trial, xs], dim=0)
...
def gen_data_loader_with_first(paradigm, probs=[1., 1., 1.], n=1):
    if paradigm == 'tst':
        contexts_to_load = [0]
        n_samples_per_context = [n]
        ds = CSWDatasetWithFirst(n_samples_per_context, contexts_to_load, probs=[1.])

    if paradigm == 'blocked':
        contexts_to_load = [0, 1, 0, 1] + [randint(0, 2) for _ in range(n)]
        n_samples_per_context = [n, n, n, n] + [1] * n
        ds = CSWDatasetWithFirst(n_samples_per_context, contexts_to_load, probs=probs)
    elif paradigm == 'interleaved':
        contexts_to_load = [0, 1] * (2 * n) + [randint(0, 2) for _ in range(n)]
        n_samples_per_context = [1] * (4 * n) + [1] * n
        ds = CSWDatasetWithFirst(n_samples_per_context, contexts_to_load, probs=probs)
