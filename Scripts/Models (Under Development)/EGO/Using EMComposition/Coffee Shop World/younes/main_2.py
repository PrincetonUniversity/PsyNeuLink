...
from pytorch_ego.run_2 import run_participant
...
def main():
    ...
    if RUN_PSY_EGO:
        ...
        pnl_results = run_model(model, pnl_inputs, config=params_ego)[:-1]
        if IS_PLOT:
            print('plotting psyneulink ego')
            plot_results(pnl_results, pnl_targets[:-1], axes[0], 1)
        if IS_VERBOSE:
    ...
    if RUN_TORCH:
        ...
        if IS_PLOT:
            ...
            plot_results(torch_results, pnl_targets[:-1], axes[1], 1)
        if IS_VERBOSE:
