...
class EMModule(nn.Module):
    ...
    def __init__(self, temperature, softmax_threshold, normalize_keys=False) -> None:
        ...
        with torch.no_grad():
            self.context_in.weight.copy_(torch.eye(11))
            #self.context_in.bias.copy_(torch.zeros(11))

        self.context_in.weight.requires_grad = True
        #self.context_in.bias.requires_grad = True
        self.context_in.requires_grad = True
    ...
    def forward(self, state: torch.tensor, context_: torch.tensor) -> torch.tensor:
        ...
        return matches#torch.clamp(matches, min=0, max=1)
...
class RecurrentContextModule(nn.Module):
    ...
    def __init__(self, n_inputs, integration_rate=.5) -> None:
        super().__init__()
        self.integration_rate = integration_rate
        self.hidden_state = torch.zeros((n_inputs,))
