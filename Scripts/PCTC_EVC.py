...
signalSearchRange = np.arange(1.0,1.1,0.2) # why 0.8 to 2.0 in increments of 0.2 np.array([1.0])#
...
bias_input = pnl.TransferMechanism(size=2,
                                   function=pnl.Linear,
                                   name='BIAS')
...
color_feature_layer = pnl.RecurrentTransferMechanism(size=2,                     # Define unit size
                                                     function=pnl.Logistic(      # Define Logistic fucntion & set gain
                                                         gain= 4, bias=1),       # to 4 & bias to 1
...
word_feature_layer = pnl.RecurrentTransferMechanism(size=2,                     # Define unit size
                                               function=pnl.Logistic(           # Define Logistic fucntion & set gain
                                                   gain= 4, bias=1),            # to 4 & bias to 1
                                                        integrator_mode=True,   # Set Integrator mode to True
                                                        smoothing_factor=Lambda,# smoothing factor ==  integration rate
...
response_layer = pnl.RecurrentTransferMechanism(size=2,                         # Define unit size
                                                # function= pnl.Logistic(         # Define Logistic fucntion & set gain
                                                #     gain= 4, bias=1),           # to 4 & bias to 1
                                                function=pnl.Logistic(
                                                    gain=4.0,
                                                    bias=(1.0, pnl.ControlProjection(
...
task_demand_layer = pnl.RecurrentTransferMechanism(size=2,                      # Define unit size
                                               function=pnl.Logistic(           # Define Logistic fucntion & set gain
                                                    bias=1,
                                                   gain= (4.0, pnl.ControlProjection(
                                                        control_signal_params={
                                                            pnl.ALLOCATION_SAMPLES: signalSearchRange}))),            # to 4 & bias to 1
                                                        integrator_mode=True,   # Set Integrator mode to True
                                                        smoothing_factor=Lambda,# smoothing factor ==  integration rate
...
# Connect mechanisms --------------------------------------------------------------------------------------------------
color_input_weights = pnl.MappingProjection(matrix=np.array([[0.0, 0.0],    # response layer projections are set to all
                                                             [0.0, 0.0]]))  # zero for initialization period first
word_input_weights = pnl.MappingProjection(matrix=np.array([[0.0, 0.0],
                                                            [0.0, 0.0]]))

color_task_weights  = pnl.MappingProjection(matrix=np.array([[2.0, 0.0],    # color to task projection
                                                             [2.0, 0.0]]))
word_task_weights  = pnl.MappingProjection(matrix=np.array([[0.0, 2.0],     # word to task projection
                                                            [0.0, 2.0]]))

task_color_weights  = pnl.MappingProjection(matrix=np.array([[1.0, 1.0],    # task to color projection
                                                             [0.0, 0.0]]))
task_word_weights  = pnl.MappingProjection(matrix=np.array([[0.0, 0.0],     # task to word projection
                                                            [1.0, 1.0]]))

color_response_weights = pnl.MappingProjection(matrix=np.array([[2.0, 0.0], # color to response projection
                                                                [0.0, 2.0]]))
word_response_weights  = pnl.MappingProjection(matrix=np.array([[2.5, 0.0], # word to response projection
                                                                [0.0, 2.5]]))

task_input_weights = pnl.MappingProjection(matrix=np.array([[1.0, 0.0],     # proactive control to task
                                                            [0.0, 1.0]]))
...
# task_conflict_to_response_weights = pnl.MappingProjection(matrix=np.array([[-1.0, -1.0]]),
...
task_color_process = pnl.Process(pathway=[task_input_layer,
                                                   task_input_weights,
                                                   task_demand_layer,
                                                   task_color_weights,
                                                   color_feature_layer,
...
task_word_process = pnl.Process(pathway=[task_input_layer,
                                                  task_demand_layer,
                                                  task_word_weights,
                                                  word_feature_layer,
                                                  word_task_weights,
...
bias_color_process = pnl.Process(pathway=[bias_input,color_feature_layer],
                                 name='BIAS_COLOR')

bias_word_process = pnl.Process(pathway=[bias_input,word_feature_layer],
                                 name='WORD_COLOR')

# conflict_process = pnl.Process(pathway=[task_demand_layer,
#                                         task_conflict_to_response_weights,
#                                         response_layer],
#                                name='CONFLICT_PROCESS')

# Create system -------------------------------------------------------------------------------------------------------
PCTC = pnl.System(processes=[word_response_process,
...
color_input_weights.matrix = np.array([[1.0, 0.0],      # set color input projections to 1 on the diagonals to e.g.
                                       [0.0, 1.0]])     # send a green color input to the green unit of the color layer
word_input_weights.matrix  = np.array([[1.0, 0.0],      # the same for word input projections
                                       [0.0, 1.0]])
...
color_input_weights.matrix = np.array([[0.0, 0.0],
                                       [0.0, 0.0]])
word_input_weights.matrix  = np.array([[0.0, 0.0],
                                       [0.0, 0.0]])
...
color_input_weights.matrix = np.array([[1.0, 0.0],      # Set input projections to 1 for stimulus presentation period
                                       [0.0, 1.0]])
word_input_weights.matrix  = np.array([[1.0, 0.0],
                                       [0.0, 1.0]])
...
color_input_weights.matrix = np.array([[1.0, 0.0],
                                       [0.0, 1.0]])
word_input_weights.matrix  = np.array([[1.0, 0.0],
                                       [0.0, 1.0]])
...
plt.setp(axes, xticks=[0,400,780],
         yticks=[0, 0.4, 0.79],
