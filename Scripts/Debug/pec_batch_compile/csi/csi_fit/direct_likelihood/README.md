...
```bash
uv run python Scripts/Debug/pec_batch_compile/csi/csi_fit/csi_direct_likelihood.py validate
```

Score the default 13-parameter vector for an actual `subject_nr` with:

```bash
uv run python Scripts/Debug/pec_batch_compile/csi/csi_fit/csi_direct_likelihood.py \
    score --subject 1
```
