...
fit_parameters = {
    ...
    ("slope", stimulusWeighting): np.linspace(0.0, 3.0, 1000),  # Stimulus to Hidden Weighting
    ("threshold", decisionMaker): np.linspace(0.01, 0.3, 1000),  # Threshold
    #("non_decision_time", decisionMaker): np.linspace(0.1, 0.4, 1000),  # Non Decision Time
}
...
for (name, mech), recovered_param in zip(fit_parameters.keys(), optimal_parameters):
    ...
    if name == "slope":
        true_param = sf_params['stim_hidden_wt']
    else:
