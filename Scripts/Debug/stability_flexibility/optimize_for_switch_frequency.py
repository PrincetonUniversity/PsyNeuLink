...
# Specify the different task-switch frequencies to optimize over
all_frequencies = np.array([0.125, 0.250, 0.375, 0.500, 0.625, 0.750, 0.875])

# Specify the number of trials and trial sequences to optimize over per task-switch frequency
num_trials = 512
num_sequences = 10
...
log_frequency = np.array([])
log_sequence = np.array([])

# Outer loop for optimization, specifying the task-switch frequency
for i_freq in all_frequencies:
    ...
    for i_seq in np.arange(num_sequences):
        ...
        taskTrain, stimulusTrain, cueTrain, correctResponse = generate_trial_sequence(num_trials, i_freq, seed=trial_seq_seed)
        ...
        print("Running the PEC for frequency ", i_freq, " and sequence ", i_seq)
        ...
        log_frequency = np.append(log_frequency, i_freq)
        log_sequence = np.append(log_sequence, i_seq)


output = pd.DataFrame({'frequency': log_frequency, 'sequence': log_sequence, 'gain': all_optimized_gain, 'threshold': all_optimized_threshold, 'rr': all_optimized_values})
output.to_csv('output/switch_frequency_optimal_gain_threshold.csv')