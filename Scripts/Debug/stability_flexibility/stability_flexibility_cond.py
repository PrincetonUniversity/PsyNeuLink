...
#%%
# For each parameter, we will add a control mechanism to the composition that overrides the parameter with a value
# from the input.
pec_mechs = {}
for (name, mech), values in fit_parameters.items():
    pec_mechs[(name, mech)] = pnl.ControlMechanism(name=f"{name}_control",
                                                   control_signals=[(name, mech)],
                                                   modulation=pnl.OVERRIDE)
    comp.add_node(pec_mechs[(name, mech)])
...
#%%
inputs_with_params = {**inputs}
for (name, mech), con_mech in pec_mechs.items():
    if name == "slope":
        value = sf_params['automaticity']
    elif name == "non_decision_time":
        value = 5.0
    else:
        value = sf_params[name]

    inputs_with_params[con_mech] = value

comp.results.clear()
comp.run(inputs_with_params, execution_mode=pnl.ExecutionMode.LLVMRun)
results = comp.results
