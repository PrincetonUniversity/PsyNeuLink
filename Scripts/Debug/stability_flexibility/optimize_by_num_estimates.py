...
# Specify the different number of estimates of the PEC to compare
...
# Specify the number of trials in a sequence, number of sequences, and how often to optimize each sequence
num_trials = 512
num_sequences = 3
num_iteration = 5

# Container for optimized estimates and corresponding outer loop values
all_optimized_gain = np.array([])
all_optimized_threshold = np.array([])
all_optimized_values = np.array([])
log_estimates = np.array([])
log_sequence = np.array([])
log_iteration = np.array([])

# Outer loop for optimization, specifying the trial sequence
for i_seq in np.arange(num_sequences):

    # Outer loop for optimization, specifying the iteration of that sequence
    for i_iter in np.arange(num_iteration):

        trial_seq_seed = i_seq
        #pnl_seed = 1
        #set_global_seed(pnl_seed)

        # Generate some model inputs
        taskTrain, stimulusTrain, cueTrain, correctResponse = generate_trial_sequence(num_trials, 0.5, seed=trial_seq_seed)
        ...
        print("Running the PEC for sequence ", i_seq, " and iteration ", i_iter)
        ...
        comp.nodes[5].output_ports[0].parameters.value.set(np.array([[1.]]), context=pnl.Context(execution_id='pec'),
                                                           override=True)

        all_optimized_gain = np.append(all_optimized_gain, pec.optimized_parameter_values[0])
        all_optimized_threshold = np.append(all_optimized_threshold, pec.optimized_parameter_values[1])
        all_optimized_values = np.append(all_optimized_values, pec.optimal_value)
        log_estimates = np.append(log_estimates, num_estimates)
        log_sequence = np.append(log_sequence, i_seq)
        log_iteration = np.append(log_iteration, i_iter)


output = pd.DataFrame({'num_estimates': log_estimates, 'sequence': log_sequence, 'iteration': log_iteration, 'gain': all_optimized_gain, 'threshold': all_optimized_threshold, 'rr': all_optimized_values})
output.to_csv('output/optimization_10_estimates.csv')