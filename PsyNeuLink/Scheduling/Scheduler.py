...
* a `System` in the **system** argument - if a System is specified,
  ...
  specified in its `executionGraph <System.executionGraph>`.

COMMENT:
    The below portion is construction by nodes and toposort_ordering, which is supported. The scheduler will not
    do any cycle checking beyond what toposort does. Additionally since in learning nodes can be Projections, it's not
    accurate to say Mechanisms
  If graph specification dictionary is used,  it must
...
A ConditionSet is simply a list of `Conditions <Condition>`.
...
**toposort_ordering** arguments, with any conflicts resolved in favor of the ConditionSet.
COMMENT
ConditionSets can also be added after a Scheduler has been created, using its `add_condition` and `add_condition_set` methods.
...
execution and that do not have any data dependencies on one another;  the order of execution of Mechanisms within a
time_step is not defined (and should not matter since, by definition, there are no data dependencies).
...
class Scheduler(object):
    """
        ...
        nodes : list[Mechanism]
            ...
            K: not always Mechanisms

        toposort : **??list?? dict??
            topological ordering of Mechanisms, **??FORMAT?  DEPENDENCY DICT?
            K: the format is a toposort generator object, the output of a toposort
