...
A Log object is a set of structures and methods for recording information generated by PsyNeuLink
`mechanisms <Mechanism>`, `processes <Process>` and `systems <System>` when they are executed.  Every instance of one of
these components has a :keyword:`log` attribute, that maintains a dictionary with entries for each
attribute of the component that has been designated to be logged.  Information is added to entries in specified
circumstances (e.g,. when the component is initialized, validated, executed, etc.) which can be specified by a
`LogLevel` specification in the components preferences; entries can also be made programmatically.  The information
in a log can be displayed using its `print_entries` method.
