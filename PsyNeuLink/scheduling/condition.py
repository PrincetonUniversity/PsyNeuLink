...
# Does it make sense to allow multiple dependencies for every_n?
# Unless all dependencies always run together, we're changing the definition of every_n
# And if they do run together, we can pick one
def every_n_calls(n, time_scale = 'trial', op = "AND"):
    """
        ...
        def check(dependencies):
            for var in dependencies:
                ...
                num_calls = {"trial": var.component.calls_current_trial,
                             "run": var.component.calls_current_run - 1,
                             "life": var.component.calls_since_initialization - 1}
                if num_calls[time_scale]%n == 0 and num_calls[time_scale] != 0:
                    return True
            return False
