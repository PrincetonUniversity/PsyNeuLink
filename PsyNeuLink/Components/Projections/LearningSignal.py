...
*Error Signal*.  The ``variable`` for a LearningSignal is used as its ``errorSignal``.



It uses this to compute the
changes to the ``matrix`` parameter of the Mapping projection for which it is responsible (i.e., the one to which it
projects).  The error signal is generated by LearningSignal's ``sender``:  it is a MonitoringMechanism that
evaluates the output of the ProcessingMechanism to which the Mapping projection being trained projects (see
...
The ``errorSignal`` is generated by a MonitoringMechanism (the LearningSignal's ``sender``), that monitors the
ProcessingMechanism to which the Mapping projection being trained projects.  The LearningSignal uses the
``errorSignal`` to compute the changes to the ``matrix`` parameter of that Mapping projection that minimize the
ProcessingMechanism's output and the target for training.  Those changes are assigned as the LearningSignal's value
