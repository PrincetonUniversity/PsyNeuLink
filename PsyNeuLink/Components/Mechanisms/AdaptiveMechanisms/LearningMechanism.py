...
class LearningMechanism(AdaptiveMechanism_Base):
    ...
    variable : 2d np.array
        has four items, each of which is a 1d np.array: `activation_input`, `activation_output`, `error_output` and
        and `error_signal`.
    ...
    error_signal : 1d np.array
        the error signal, typically generated by the `LearningMechanism` associated with the `error_output` (i.e.,
        the one projected to by the mechanism that receives the `MappingProjection` being learned).  If the
        LearningMechanism is for the last mechanism in a sequence of mechanisms being learned (often, but not
