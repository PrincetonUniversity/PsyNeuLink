...
to "train" a single MappingProjection (its `primary_learned_projection`), using the output of the mechanism to which
that MappingProjection projects (its `output_source`) as the source of the error it attempts to reduce.  However,
a LearningMechanism can be used to train multiple MappingProjections, by assigning it additional LearningProjections;
however, these will all use the same error signal, generated by the `primary_learned_projection` and its associated
`output_source`.
...
* *ERROR_SIGNAL* - receives the value of an `error_signal <LearningMechanism.error_signal>` from either a
  ...
  sequence (i.e., the one associated with the `output_source`).  It is used by the LearningMechanism's `function
  <LearningMechanism.function>` to calculate the `learning_signal <LearningMechanism.learning_signal>`;  note that
  the value of the *ERROR_SIGNAL* InputState may not be the same as that of the `error_signal
  <LearningMechanism.error_signal>` attribute or *ERROR_SIGNAL* OutputState (see `below
  <LearningMechanism_Error_Signal>`). The `value <InputState.value>` of the *ERROR_SIGNAL*
...
* `error_signal <LearningMechanism.error_signal>` - the contribution made by the `primary_learned_projection` to the
  ...
  .. note::
     ...
     *ERROR_SIGNAL** `OutputState <LearningMechanism_Output_Error_Signal>` may not have the same value.
     ...
     while the latter is the contribution made to that error by the `primary_learned_projection` and its
     associated output, as calculated by the LearningMechanism's `function <LearningMechanism.function>`.
...
* *ERROR_SIGNAL* - this receives the value of the `error_signal <LearningMechanism.error_signal>` used to calculate
  the `learning_signal <LearningMechanism.learning_signal>`.  Note that although that its value may not be the same
  as that of the *ERROR_SIGNAL* `InputState <LearningMechanism_Input_Error_Signal>` (see `above
  <LearningMechanism_Error_Signal>`).  The *ERROR_SIGNAL* OutputState is always the LearningMechanism's first (i.e.,
  `primary <OutputState_Primary>`) OutputState, and is always named *ERROR_SIGNAL*.
...
* `LearningSignal(s) <LearningSignal>` - by default, a LearningMechanism has a single LearningSignal, which is a
  special type of OutputState that receives `learning_signal <LearningMechanism.learning_signal>` (usually a matrix
  of weight changes) generated by the LearningMechanism's `function <LearningMechanism.function>`, and used to
  modify the `matrix <MappingProjection.matrix>` parameter of the `primary_learned_projection`.  The LearningSignal
  is assigned as the second item in the list of the LearningMechanism's OutputStates
  (i.e., of its `output_states <LearningMechanism.output_states>` attribute), and its `value <LearningSignal.value>`
  is assigned as the second item of the LearningMechanism's `output_values <LearningMechanism.output_values>` attribute.
...
..
* `input_source` - the `Mechanism` that sends the `primary_learned_projection`, and provides the input to the
...
class LearningMechanism(AdaptiveMechanism_Base):
    ...
    error_signal : 1d np.array
        ...
        LearningMechanism's *ERROR_SIGNAL* `InputState <LearningMechanism_Input_Error_Signal>`;  for `multilayer
    ...
    learning_signals : List[LearningSignal]
        ...
        *ERROR_SIGNAL* `OutputState <LearningMechanism_Output_Error_Signal>`, and any others it may have.
