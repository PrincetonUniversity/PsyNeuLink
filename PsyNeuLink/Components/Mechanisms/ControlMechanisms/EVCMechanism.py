...
the outputStates and the control costs generated by that policy.  It then selects the policy that yields the
greatest value, and allocates the ControlSignal values for that policy. This implements a form of the EVC Theory
described in Shenhav et al. (2013).
...
details of parameter specification):
    ...
    * :keyword:`PREDICTION_MECHANISM_TYPE` - specifies the type of prediction mechanism used to generate the input
      to the system for each simulation run by the EVCMechanism (see :ref:`EVCMechanism_Execution`).
        ...
        COMMENT
    ..
    * :keyword:`PREDICTION_MECHANISM_PARAMS` - specifies the parameters for the prediction mechanism.
        COMMENT:
           ADD TO ARG AND/OR ATTRIBUTE DESCRIPTION BELOW
...
COMMENT:
    Aloocation policies and range
    Objective function and how it is handled
    Simulation(s), incuding predictions
