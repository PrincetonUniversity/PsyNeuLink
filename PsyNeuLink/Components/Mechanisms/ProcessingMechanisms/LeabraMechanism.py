...
from PsyNeuLink.Globals.Keywords import FUNCTION, GAIN, INITIALIZER, INITIALIZING, LEABRA_FUNCTION, LEABRA_FUNCTION_TYPE, LEABRA_MECHANISM, MEAN, MEDIAN, NOISE, RATE, RESULT, STANDARD_DEVIATION, TRANSFER_FUNCTION_TYPE, TRANSFER_MECHANISM, VARIANCE, kwPreferenceSetName
...
class LeabraFunction(Function_Base):
    ...
    def __init__(self,
        ...
        self.network = emergent_network
        default_variable = np.zeros(emergent_network.layers[0].size) # NOTE: check with Fabien if input_size property exists
    ...
    def _validate_variable(self, variable, context=None):
        if not isinstance(variable, (list, np.ndarray, numbers.Number)):
            raise LeabraError("Input Error: variable ({}) was of type {}, but instead should be a list, numpy array, or"
                              " number.".format(variable, type(variable)))
        return variable

    def _validate_params(self, request_set, target_set=None, context=None):
        # add check that emergent_network is a leabra network
...
class LeabraMechanism(ProcessingMechanism_Base):
    ...
    def __init__(self,
        ...
        leabra_function = LeabraFunction(emergent_network=leabra_network)  #, owner=self)

        # size = [input_size, input_size]
        # input_states = ['main_input', 'learning target']

        params = self._assign_args_to_param_dicts(function=leabra_function,
