...
pec_test_args = [
    (
        ...
        True,
        False,
    ),  # No ObjectiveMechanism, 2 inputs, model, no nodes or pathways arg
    (
        None,
        2,
        False,
        True,
    ),  # No ObjectiveMechanism, 2 inputs, no model, nodes or pathways arg
    (
        Concatenate,
        2,
        True,
        False,
    ),  # ObjectiveMechanism, 2 inputs, model, no nodes or pathways arg
    (
        LinearCombination,
        1,
        True,
        False,
    ),  # ObjectiveMechanism, 1 input, model, no nodes or pathways arg
...
@pytest.mark.parametrize(
    ...
    pec_test_args,
    ids=[
        f"{x[0]}-{'model' if x[2] else None}-{'nodes' if x[3] else None})"
...
def test_pec(objective_function_arg, expected_outcome_input_len, model_spec, node_spec):
    ...
    Decision = DDM(
        ...
        output_ports=[DECISION_VARIABLE, RESPONSE_TIME, PROBABILITY_UPPER_THRESHOLD],
        name="Decision1",
    )
    Decision2 = DDM(
        function=DriftDiffusionAnalytical(
    ...
    assert (
        ctlr.control_signals[pnl.RANDOMIZATION_CONTROL_SIGNAL].allocation_samples.num
        == 3
    )
