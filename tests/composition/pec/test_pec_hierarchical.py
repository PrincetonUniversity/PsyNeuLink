...
def test_a_full_group_covariance_needs_a_full_curvature():
    ...
    with pytest.raises(ValueError, match="needs curvature"):
        fit_laplace_em(runner, model.n_subjects, model.n_params,
                       estep_config=EStepConfig(curvature="diagonal"), covariance="full",
                       max_iterations=1)
...
def test_em_history_pairs_each_objective_with_the_estimate_that_produced_it():
    ...
    np.testing.assert_allclose(result.history[0]["covariance"], np.diag(init_covariance))
...
@pytest.mark.composition
def test_asking_for_group_correlations_needs_a_curvature_that_measured_them():
    # Either setting is allowed on its own, so the pair is refused where a fit reads them
    # together -- which is also the last point at which either could still be changed.
    pec = _build_group_pec(
        distributed_options={"pec_factory": _stub_factory},
        hierarchical_options={
            "subject_id": "subject", "covariance": "full", "curvature": "diagonal",
            "max_iterations": 1,
        },
    )
    with pytest.raises(ParameterEstimationCompositionError, match="needs curvature"):
        pec.run()
