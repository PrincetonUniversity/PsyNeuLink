...
class TestProjectionSpecificationFormats:
    ...
    def test_multiple_modulatory_projection_specs(self):
        ...
        assert M.parameter_ports[
                   psyneulink.core.components.functions.nonstatefulfunctions.distributionfunctions.DRIFT_RATE].mod_afferents[0] == C.control_signals[0].efferents[0]
        assert M.parameter_ports[
    ...
    def test_2_item_tuple_from_control_signal_to_parameter_port(self):
        ...
        C = pnl.ControlMechanism(control_signals=[(
            psyneulink.core.components.functions.nonstatefulfunctions.distributionfunctions.DRIFT_RATE, D)])
    ...
    def test_2_item_tuple_from_parameter_port_to_control_signals(self):
        ...
        D = pnl.DDM(name='D3',
                     function=psyneulink.core.components.functions.nonstatefulfunctions.distributionfunctions.DriftDiffusionAnalytical(drift_rate=(3, C),
                                                                                                                                       threshold=(2,C.control_signals['b']))
                    )
        assert D.parameter_ports[
                   psyneulink.core.components.functions.nonstatefulfunctions.distributionfunctions.DRIFT_RATE].mod_afferents[0].sender == C.control_signals[0]
        assert D.parameter_ports[
    ...
    def test_formats_for_control_specification_for_mechanism_and_function_params(self, noise, gain):
        ...
        R = pnl.RecurrentTransferMechanism(
            ...
            name='R-CONTROL',
            noise=noise,
            function=psyneulink.core.components.functions.nonstatefulfunctions.transferfunctions.Logistic(gain=gain)
        )
