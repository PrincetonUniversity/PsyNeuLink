...
class TestProjectionSpecificationFormats:
    ...
    def test_projection_specification_matrix_dict(self, init_accumulator_rate):
        ...
        # rate=1 should pass this even if there is an extra integration step during init,
        # because it won't change the AccumulatorIntegrator's previous_value
    ...
    def test_masked_mapping_projection_mask_conficts_with_matrix(self):
        error_msg = "Shape of the 'mask' for MaskedMappingProjection-0 ((2, 3)) must be the same as its 'matrix' ((2, 2))"
        with pytest.raises(pnl.MaskedMappingProjectionError, match=re.escape(error_msg)):
    ...
    #
    #     with pytest.warns(UserWarning, match=re.escape("that already has an identical Projection")):
    ...
    def test_duplicate_projection_creation_error(self):
        msg = "Attempt to assign Projection to 'InputPort-0' of 'T2' that already has an identical Projection."
        with pytest.raises(pnl.DuplicateProjectionError, match=re.escape(msg)):
            T1 = pnl.TransferMechanism(name='T1')
            T2 = pnl.TransferMechanism(name='T2')
