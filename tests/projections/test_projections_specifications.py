...
class TestProjectionSpecificationFormats:
    ...
    def test_multiple_modulatory_projection_specs(self):
        ...
        assert M.parameter_states[
                   psyneulink.core.components.functions.statefulfunctions.integratorfunctions.DRIFT_RATE].mod_afferents[0] == C.control_signals[0].efferents[0]
        assert M.parameter_states[
                   psyneulink.core.components.functions.statefulfunctions.integratorfunctions.THRESHOLD].mod_afferents[0] == C.control_signals[0].efferents[1]
    ...
    def test_2_item_tuple_from_control_signal_to_parameter_state(self):
        ...
        # Single name
        C = pnl.ControlMechanism(control_signals=[(psyneulink.core.components.functions.statefulfunctions.integratorfunctions.DRIFT_RATE, D)])
        ...
        # List of names
        C = pnl.ControlMechanism(control_signals=[([psyneulink.core.components.functions.statefulfunctions.integratorfunctions.DRIFT_RATE,
                                                    psyneulink.core.components.functions.statefulfunctions.integratorfunctions.THRESHOLD], D)])
    ...
    def test_2_item_tuple_from_parameter_state_to_control_signals(self):
        ...
        D = pnl.DDM(name='D3',
                     function=psyneulink.core.components.functions.statefulfunctions.integratorfunctions.DriftDiffusionAnalytical(drift_rate=(3, C),
                                                                                                                threshold=(2,C.control_signals['b']))
                    )
        assert D.parameter_states[
                   psyneulink.core.components.functions.statefulfunctions.integratorfunctions.DRIFT_RATE].mod_afferents[0].sender == C.control_signals[0]
        assert D.parameter_states[
                   psyneulink.core.components.functions.statefulfunctions.integratorfunctions.THRESHOLD].mod_afferents[0].sender == C.control_signals[1]
    ...
    def test_duplicate_projection_detection_and_warning(self):
        ...
        for warning in record:
            if "already exists; will ignore additional one specified" in str(warning.message):
