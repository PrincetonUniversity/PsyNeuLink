...
for sim=1:nr_simulations
    ...
    for t=1:nr_training_trials
        ...
        drift_rates=1+control_signals(2);
        ...
        if p_error>1
            throw(MException('MException','Impossibly high error rate.'))
        end
    ...
    for t=1:nr_test_trials
        ...
        chosen_signal1(t)=control_signals(1);
        chosen_signal2(t)=control_signals(2);
    end   
    
    avg_control_signal1(sim,1)=mean(chosen_signal1(mostly_congruent));
    avg_control_signal1(sim,2)=mean(chosen_signal1(mostly_incongruent));
    avg_control_signal2(sim,1)=mean(chosen_signal2(mostly_congruent));
    avg_control_signal2(sim,2)=mean(chosen_signal2(mostly_incongruent));
...
mean(avg_control_signal1)
mean(avg_control_signal2)
