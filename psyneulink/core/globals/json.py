...
Any PsyNeuLink `Composition` or `Component` can be exported to the JSON format using its `json_summary` method,
which uses its `_dict_summary <Component._dict_summary>`.  This generates a string that, passed into
the `generate_script_from_json` function, produces a valid Python script replicating the original PsyNeuLink model.
`write_json_file` can be used to write the json_summary for one or more Compositions into a specified file.
`generate_script_from_json` can accept either the string returned by `generate_script_from_json` or the name
of a file containing one.  Calling ``exec(generate_script_from_json(<input>))`` will load into the current namespace
all of the PsyNeuLink objects specified in the ``input``; and `get_compositions` can be used to retrieve a list of
all of the Compositions in that namespace, including any generated by execution of `generate_script_from_json`.

.. _JSON_Security_Warning:a
