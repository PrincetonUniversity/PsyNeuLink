...
def _load_wi(builder, idx, fptype):
    ...
    global_wi.initializer = fmt_ty(_wi_double_data)

    ptr = builder.gep(global_wi, [idx.type(0), idx])
    return builder.load(ptr)


def _load_ki(builder, idx, itype):
    module = builder.function.module
    ...
    global_ki.initializer = fmt_ty(_ki_i64_data)

    ptr = builder.gep(global_ki, [idx.type(0), idx])
    return builder.load(ptr)


def _load_fi(builder, idx, fptype):
    module = builder.function.module
    fmt_ty = ir.ArrayType(fptype, 256)
    ...
    try:
        ...
        global_fi.initializer = fmt_ty(_fi_double_data)
...
def _setup_philox_rand_normal(ctx, state_ty, gen_float, gen_int):
    ...
    wi = _load_wi(builder, idx, fptype)
    ...
    # idx has to be > 0
    fi_idx = _load_fi(builder, idx, fptype)
    fi_idxm1 = _load_fi(builder, builder.sub(idx, idx.type(1)), fptype)
...
def setup_philox(ctx):
    ...
    _setup_philox_rand_normal(ctx, state_ty, gen_double, gen_int64)
