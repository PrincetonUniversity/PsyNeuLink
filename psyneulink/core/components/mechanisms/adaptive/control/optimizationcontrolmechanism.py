...
combination of the `costs <ControlMechanism.costs>` associated with the `allocation_policy
<ControlMechanism.allocation_policy>` and the current state, depending on the particular subclass.  The table `below
...
<trial>` to predict the EVC based on previously experienced combinations of input and `allocation_policy
<ControlMechanism.allocation_policy>` and, within a given trial, uses the FunctionApproximator's `make_prediction
<FunctionApproximator.make_prediction>` method to test `allocation_policies <ControlMechanism.allocation_policy>`
and find the one that is predicted to yield the greatest EVC for the current (or expected) input.  In contrast,
the `model-based <ModelBasedOptimizationControlMechanism>` class finds the `allocation_policy
<ControlMechanism.allocation_policy>` that yields the greatest EVC by actually testing them within a trial,
...
allocation_policy that yields the greatest `EVC <OptimizationControlMechanism_EVC>` for the current (or expected)
...
is used to evaluate each `allocation_policy <ControlMechanism.allocation_policy>` sampled by the `search_function
...
find the `allocation_policy <ControlMechanism.allocation_policy>` that yields the greatest `EVC
<OptimizationControlMechanism_EVC>`.  The way in which it searches for the best `allocation_policy
<ControlMechanism.allocation_policy>` is determined by the type of `OptimzationFunction` assigned to `function
...
specifically, The `function <OptimizationControlMechanism.function>` selects a sample `allocation_policy
<ControlMechanism.allocation_policy>` (usually using `search_function <OptimizationControlMechanism.search_function>`
to select one from `allocation_policy_search_space <OptimizationControlMechanism.allocation_policy_search_space>`),
and evaluates the EVC for that `allocation_policy <ControlMechanism.allocation_policy>` using the
...
class uses a `FunctionApproximator` to predict find the `allocation_policy <ControlMechanism.allocation_policy>` that
...
method to execute the Composition with each `allocation_policy` and determine the actual outcome for the current (or
expected) input.  In either case, the values of the `allocation_policy <ControlMechanism.allocation_policy>` that
...
class OptimizationControlMechanism(ControlMechanism):
    ...
    def _execute(self, variable=None, runtime_params=None, context=None):
        ...
        self.feature_values = self.agent_rep.before_execution(context=self.context)
        # FIX END MODEL_FREE

        # FIX: MODEL_BASED
        self.predicted_input = self.agent_rep.before_simulations()
        # FIX: END MODEL_BASED

        # Compute allocation_policy using ModelFreeOptimizationControlMechanism's optimization function
        # IMPLEMENTATION NOTE: skip ControlMechanism._execute since it is a stub method that returns input_values
        allocation_policy, self.metric_optimal, self.saved_samples, self.saved_values = \
