    ...
    def reset(self, *args, force=False, context=None, **kwargs):
        ...
        # (3) update value, (4) update output ports
            ...
            if self.parameters.integrator_mode._get(context) or force:
                # reset returns one entry per stateful_attribute of the
                # integrator_function (e.g. previous_v, previous_w and
                # previous_time for FitzHughNagumoIntegrator).  For integrators
                # with a single stateful_attribute (the common case), unwrap it
                # so the function operates on that value directly.  For
