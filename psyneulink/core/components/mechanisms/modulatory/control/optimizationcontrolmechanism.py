...
* **state_features** -- specifies the values provided by the OptimizationControlMechanism as the input to its
  ...
  **state_features** *for an agent_rep that is a* **CompositionFunctionApproximator**
  ...
  `evaluate
  <CompositionFunctionApproximator.evaluate>` method. These cannot be determined automatically and so
  they *must be
  specified explicitly*, in a list, with the correct number of items in the same order and with
  the same shapes they
