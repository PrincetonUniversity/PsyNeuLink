...
def simulation_likelihood(sim_data,
    ...
    for trial in range(len(con_sim_data)):
        ...
        for category in categories:
            ...
            data_range = np.max(dsub) - np.min(dsub) if dsub.ndim == 1 else np.amax(dsub, 1) - np.amin(dsub, 1)
            if np.any(data_range == 0):
                dens_u[category] = (None, None)
                warnings.warn(BadLikelihoodWarning(
        ...
        for trial in range(len(exp_data)):
            ...
            if kde is not None:
                kdes_eval[trial] = interpn(axes, kde, exp_data[trial, ~categorical_dims],
                                           method='linear', bounds_error=False, fill_value=ZERO_PROB)
            else:
...
class MaxLikelihoodEstimator:
    ...
    def __init__(self,
        ...
        self.IterRecord = collections.namedtuple('IterRecord',
    ...
    def fit(self,
        ...
        with Progress(
            ...
            with warnings.catch_warnings(record=True) as warns:
                ...
                def neg_log_like(x):
                    ...
                    if display_iter:
                        ...
                        if len(warns) > 0 and warns[-1].category == BadLikelihoodWarning:
                            ...
                            progress.console.print(f"{get_param_str(params)}, Neg-Log-Likelihood: {p}, "
                                                   f"Likelihood-Eval-Time: {elapsed} (seconds)", style="bold red")
                            # Clear the warnings
                            warns.clear()
                        else:
                            progress.console.print(f"{get_param_str(params)}, Neg-Log-Likelihood: {p}, "
                    ...
                    if save_iterations:
                        iterations.append(self.IterRecord(**params, neg_log_likelihood=p, likelihood_eval_time=elapsed))
                ...
                def progress_callback(x, convergence):
                    ...
                    progress.console.print(f"[green]Current Best Parameters: {get_param_str(params)}, Neg-Log-Likelihood: {neg_log_like(x)}")

                    # If we encounter any BadLikelihoodWarnings. Summarize them for the user
                    if len(warns_with_params) > 0:
                        progress.console.print("Warning: degenerate likelihood for the following parameter values ", style='bold red')
                        for w in warns_with_params:
                            progress.console.print(f"\t{get_param_str(w[1])}", style='bold red')
                        progress.console.print("If these warnings are intermittent, check to see if search "
                ...
                r = differential_evolution(neg_log_like, bounds, callback=progress_callback, maxiter=500, polish=False)
        ...
        if save_iterations:
            output_dict['iterations'] = pd.DataFrame.from_records(iterations, columns=self.IterRecord._fields)
