"""Batched scalar first-passage solver with a Torch oracle and fused CPU adjoint."""
...
class MovingBoundaryDDMSolver:
    ...
    def solve_observation_batch(
        ...
        *,
        ...
        # A value-only call must use the same fused path even when the caller
        # did not explicitly enter no_grad(). Avoid building a Torch step graph
        # (or allocating the density tape) when no input needs a derivative.
...
class _DifferentiableMovingBoundarySolve(torch.autograd.Function):
    ...
    def backward(
        ...
        if ctx.solver.native_forward and torch.is_grad_enabled():
            raise RuntimeError(
                "The native first-passage adjoint supports first-order derivatives only; "
                "use create_graph=False. Higher derivatives require a different backend."
            )
        del (
            gradient_upper,
            gradient_lower,
