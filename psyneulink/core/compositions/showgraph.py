...
def _assign_processing_components(g, rcvr, enclosing_g, show_nested, nesting_level, attrs):
    ...
    elif isinstance(rcvr, Composition) and attrs.show_nested is not NESTED:
        if rcvr in attrs.active_items:
            if attrs.active_color == BOLD:
...
# def _assign_cim_components(g, cims, enclosing_g, attrs):
#
#     cim_rank = 'same'
#
#     for cim in cims:
...
#                                      color=proj_color, penwidth=proj_width)
#
# def _assign_controller_components(g, attrs):
...
#
# def _assign_learning_components(g, attrs):
...
#
# @tc.typecheck
# def _assign_incoming_edges(g, rcvr, rcvr_label, senders, proj_color=None, proj_arrow=None, enclosing_g=None, attrs):
#
