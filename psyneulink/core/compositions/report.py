...
class ReportOutput(Enum):
    """
    Denotes enabling and type of output generated by `report_output <Report.report_output>` as specified in the
    **report_output** argument in any of a `Composition`\'s `execution methods <Composition_Execution_Methods>` or
    the `execute <Mechanism_Base.execute>` of a `Mechanism`.

    .  If **report_output** is
    specified as `USE_PREFS <ReportOutput.USE_PREFS>` or `TERSE <ReportOutput.TERSE>`, the information is reported
    to the the devices specified in `report_to_devices <Report.report_to_devices>` as it is generated;  if *FULL* is
    specified, then the information is reported at the
    end of each `TRIAL <TimeScale.TRIAL>` of execution formatted using `rich Panel objects
    <https://rich.readthedocs.io/en/stable/panel.html>`_
    ...
    USE_PREFS
        uses `Component` `reportOutputPref <PreferenceSet_reportOutputPref>` to determine whether it is included in
        the output report; each item is reported as it is executed in the same format as the TERSE option (see below).

    TERSE
        enforces reporting of execution of *all* Components as they are executed, irrespective of their
        `reportOutputPref <PreferenceSet_reportOutputPref>` setting, using a simple line-by-line format to report each.

    FULL
        enforces reporting of execution *all* Components at the end of each `TRIAL <TimeScale.TRIAL>` of
        execution, including the input and output of each Component, formatted using `rich Panel objects
        <https://rich.readthedocs.io/en/stable/panel.html>`_.
...
class ReportDevices(Flag):
    ...
    CONSOLE
        directs reporting to the console in which PsyNeuLink is running
    ...
    RECORD
        captures reporting in `_recorded_reports <Report._recorded_reports>`; specifying this
    ...
    .. _technical_note::
        DIVERT
            captures reporting otherwise directed to the rich Console in a UDF-8 formatted string and
    ...
    PNL_VIEW
        directs reporting to the PsyNeuLinkView graphical interface [UNDER DEVELOPMENT].
...
class Report:
    """
    Provides interface to output and progress reporting.

    .. technical_note::
        This is a singleton context object, that returns the currently active progress context instance if one has been
        instantiated already in another scope. It deallocates the progress bar when the outermost context is released.

    **Output reports** provide information about the input and output to a `Mechanism` or to a `Composition` and
    ...
    Mechanism's <Mechanism_Base.execute>` method or any of a Composition's `execution methods
    <Composition_Execution_Methods>`.

    **Progress reports** provide information about the status of execution of a Composition's `run <Composition.run>`
    or `learn <Composition.run>` methods, and are updated at the end of each `TRIAL <TimeScale.TRIAL>` of execution
    using the `rich Progress Display <https://rich.readthedocs.io/en/stable/progress.html#>`_.  They can be
    enabled/disabled using the **report_progress** argument of either of those methods.

    Output and progress reports can include information about execution in `simulations
    <OptimizationControlMechanism_Execution>` executed by an a Composition's `controller <Composition_Controller>`)
    by specifying the **report_simulatons** argument of a Composition's `run <Composition.run>` or `learn
    <Composition.run>` methods.

    The device(s) to which reporting is sent can be specified using the **report_to_device** argument of a Mechanism's
    <Mechanism_Base.execute>` method or any of a Composition's `execution methods <Composition_Execution_Methods>`;
    ...
    report_to_devices : list(ReportDevices) : default CONSOLE
        specifies where output and progress should be reported;  see `ReportDevices` for options.
    ...
    def report_output(self, caller,
        ...
        report_num : int
            id of `ProgressReport` (stored in `_progress_reports <Report._progress_reports>`.
