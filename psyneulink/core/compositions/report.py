...
class ReportOutput(Enum):
    """
        Denotes type of output generated by `report_output <Report.report_output>`.
    ...
    TERSE
        enforces reporting of execution *all* Components, using a simple line-by-line format to report each.

    FULL
        input and output of all Components executed is reported, using `rich Panel objects
        <https://rich.readthedocs.io/en/stable/panel.html>`_.
...
class Report:
    ...
    **Progress reports** provide information about the status of execution of a Composition's `run <Composition.run>`
    ...
    report_simulations : bool : default False
        specifies whether to show output and progress for `simulations <OptimizationControlMechanism_Execution>`
