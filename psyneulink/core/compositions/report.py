...
They can be enabled/disabled using the **report_progress** argument of either of those methods.
...
class ReportOutput(Enum):
    ...
    `TERSE <ReportOutput.TERSE>` is used, reporting is sent to the the devices specified in the **report_to_devices**
    argument as it is generated during execution; if *FULL* is used, then the information is reported at the end of
    each `TRIAL <TimeScale.TRIAL>` executed.
    ...
    USE_PREFS
        uses the `reportOutputPref <PreferenceSet_reportOutputPref>` of each `Composition` and/or `Mechanism` executed
        to determine whether and in what format to report its execution.

    TERSE
        enforces reporting execution of *all* Compositions and/or Mechanisms as they are executed, irrespective of their
        `reportOutputPref <PreferenceSet_reportOutputPref>` settings, using a simple line-by-line format to report each.

    FULL
        enforces formatted reporting execution of *all* Compositions and/or Mechanisms at the end of each
...
class Report:
    ...
    report_progress : bool : default False
        ...
        showing the number of `TRIALS <TimeScale.TRIAL>` that have been executed and a progress bar. If the number
        `TRIALS <TimeScale.TRIAL>` to be executed is determinable (e.g., the **num_trials** of a Composition's `run
        ...
        displayed once complete.  Progress is reported to the devices specified in the **report_to_devices** argument.
