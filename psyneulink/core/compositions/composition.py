...
Composition:  using standard PsyNeuLink Components; or by using `AutodiffComposition`, a specialized subclass
of Composition that executes learning using `PyTorch <https://pytorch.org>`_.  The advantage of the latter is that
it executes more quickly; however, native PsyNeuLink implementation provides a more fully exposed view of the Components
involved in learning, including those responsible for computing errors, and for calcuating and applying changes to the
matrix parameter of MappingProjections being learned (see `LearningMechanisms_Note` for further discussion).
Each of these approaches is described below.

.. _Composition_Learning_Native

Learning Using PsyNeuLink Components
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Learning is generally divided into two broad classes:  unsupervised, in which a model tries to discover underlying
structure and/or relationships among the inputs on its own;  and supervised, which modifies the model to generate a
desired output (target) for each input (see `<https://www.geeksforgeeks.org/supervised-unsupervised-learning/>`_
for a useful summary).  Both can be implemented in PsyNeuLink, and rely on the use of `LearningMechanisms
<LearningMechanism>` (that compute the changes to make to the `matrix <MappingProjection.matrix>` parameter of
`MappingProjections <MappingProjection>` being learned) and `LearningProjections <LearningProjection>` (that apply
those changes to the MappingProjections).  In addition, supervised learning requires the use of a `ComparatorMechanism`
that computes the error (the between the reponse generated by the Composition and the specified target).  In most
cases, the LearningMechanisms, LearningProjections and, if necesssary, ComparatorMechanism are generated automatically,
as described for each form of learning below.
