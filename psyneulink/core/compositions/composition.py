...
class Composition(Composition_Base, metaclass=ComponentsMeta):
    ...
    def run(
        ...
        if initialize_cycle_values is not None:
            self.initialize_cycles(values=initialize_cycle_values, include_unspecified_nodes=False, context=context)
    ...
    def initialize_cycles(self, values=None, include_unspecified_nodes=True, context=NotImplemented):
        """
        ...
        for node in values:
            if node not in self.nodes:
                raise CompositionError(f"{node.name} "
                                       f"(entry in initialize_cycles values arg) is not a node in '{self.name}'")
