...
.. _Composition_Learning_Figure:

**Learning Components**

.. figure:: _static/Composition_Learning_fig.svg
   ...
   components (capitalized in italics) returned by the learning method (see `BasicsAndSampler_Learning`
   for an example of such a network that learns to compute the XOR function).
...
^^^^^^^^^^^^^^^^^^^^^
When a Composition is run that contains one or more learning sequences, first all of the ProcessingMechanisms in the
sequence are executed, and then its LearningComponents.

• ADD DESCRIPTION OF EXECUTION:
    FIRST PROCESING MECHANISM, THEN LEARNING COMPONENTS -
• ADD ANIMATION OF XOR EXAMPLE IN BasicsAndSampler
