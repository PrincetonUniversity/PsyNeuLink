...
name of the item as the key for each entry, and the object(s) as its value):
    ...
    * `TARGET_MECHANISM` -- receives the value to be used by the `COMPARATOR_MECHANISM` as the target in
      ...
      Composition's `run <Composition.run>` method (as the input to the TARGET_MECHANISM;
    ..
...
If the learning sequence involves more than two ProcessingMechanisms (e.g. using `add_backpropagation_pathway` for a
