...
Components listed below, and assigns them the `NodeRoles <NodeRole>` indicated (see
`LearningMechanism_Single_Layer_Learning` for a more detailed description and figure showing these Components);
the items with names below are returned by the learning method in a dictionary, in which each name is the key of an
entry, and the object(s) created of that type are its value.
...
for a multilayered neural network), then an additional LearningMechanisms are created, along with MappingProjections
...
in the sequence (see `LearningMechanism_Multilayer_Learning` for a more detailed description and a figure showing such
a configuration).  These additional learning components are listed in the `LEARNING_MECHANISM` and `LEARNED_PROJECTION`
entries of the dictionary returned by the learning method.

COMMENT:
• REFERENCE TO FIGS IN LEARNINGMECHANISM, MAYBE WITH SIMPLIFIED VERSION OF THE FOLLOWING HERE (FROM PROCESS):
...
<LearningProjection_Output_vs_Terminal_Figure>` for an example).  Furthermore, if it *is* the last Mechanism in all of
...
.. _LearningProjection_Output_vs_Terminal_Figure:

    **OUTPUT** vs. **TERMINAL** Roles in Learning Configuration

    .. figure:: _static/LearningMechanism_OUTPUT_vs_TERMINAL_fig.svg
       ...
       (e.g., ``add_backpropagation_learning_pathway(pathway=[A,C])`` and
       ``add_backpropagation_learning_pathway(pathway=[B,C,D])``).  Mechanism C is the last Mechanism of the
       sequence specified for the first pathway and so, if that were created on its own, Mechanism C would
       projection to a `ComparatorMechanism` and would be assigned as an `OUTPUT` node of the Composition.
       However, since it appears in the middle of the sequence specified for the second pathway, it does not
       project to a ComparatorMechanism and is relegated to being an `INTERNAL` node of the Composition;
       Mecahnism D is now the Mechanism that projects to the ComparatorMechanism, and assigned as the
       `OUTPUT` node.
        ...
        for efferent in [p for p in receiver_activity_mech.efferents
            ...
            for learning_projection in [mod_aff for mod_aff in efferent.parameter_states[MATRIX].mod_afferents
                ...
                if learning_mech:
                    ...
                    #    error_signal_input_state of learning_mech in _create_multilayer_backprop_components
