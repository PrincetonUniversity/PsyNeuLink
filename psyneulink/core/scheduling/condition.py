...
--------
  ...
  * `Condition_Overview`
     - `Subtractive Conditions <Condition_Overview_Subtractive>`
     - `Additive Conditions <Condition_Overview_Additive>`
  * `Condition_Creation`
     - `Condition_Pre_Specified`
     - `Condition_Custom`
        - `Condition_Custom_Subtractive`
        - `Condition_Custom_Additive`
  * `Condition_Structure`
     - `Condition_Pre-Specified_List`
        - `Generic <Conditions_Generic>`
        - `Static <Conditions_Static>`
        - `Composite <Conditions_Composite>`
        - `Time-Based Subtractive <Conditions_Time_Based_Subtractive>`
        - `Time-Based Additive <Conditions_Time_Based_Additive>`
        - `Component-Based <Conditions_Component_Based>`
        - `Context-Based <Conditions_Context_Based>`
        - `Convenience <Conditions_Convenience>`

  * `Condition_Execution`
     - `Condition_Execution_Additive`
     - `Condition_Execution_Subtractive`
  * `Condition_Class_Reference`
...
`Conditions <Condition>` are used to customize when a `Component` executes.  By default, a Component executes exactly
once after all of the `nodes <Composition_Nodes>` that project to it execute.  However, Conditions can be used to
modify this  behavior. There are two types of Condition:  the `Subtractive <ConditionType>` `condition_type`,
that restricts execution of a Component to a specified circumstance or set of circumstances;  and the `Additive
<ConditionType>` `condition_type`, that forces a Component to execute a specified number of times.  For a Condition to
take effect, it must be added to the `Scheduler` for the `Composition` in which the Component is to be executed.

.. _Condition_Overview_Subtractive:

*Subtractive Conditions*
~~~~~~~~~~~~~~~~~~~~~~~~

These are used to specify criteria that must be met for a Component to execute.  They are called "subtractive" as
they are likely to restrict the number of times a Component will execute relative to the default, and never increase
the number.  Subtract conditions can specify the state of the Component itself (e.g., how many times it has already
executed, or the value of one of its attributes), the state of the Composition (e.g., how many `TIME_STEP` s have
occurred in the current `TRIAL <TimeScale.TRIAL>`), and/or the state of other Components in a Composition (e.g.,
whether or how many times they have executed). PsyNeuLink provides
a number of `pre-specified Conditions <Condition_Pre_Specified>` that can be parametrized (e.g.,  how many times a
Component should be executed). `Custom conditions <Condition_Custom>` can also be created, by assigning a function to
a Condition that can reference any Component or its attributes in PsyNeuLink, thus providing considerable flexibility
for scheduling.

.. _Condition_Overview_Additive:
...
HAVING A HARD TIME FOLLOWING THIS
 -- AS STATED, NEED TO UNDERSTAND HOW THE SCHEDULER WORKS (AND EVEN WHAT IT IS) TO UNDERSTAND THIS.  IS THERE A WAY
    TO EXPLAIN IT WITHOUT REFERENCING consideration)_queue and consideration set?  IF NOT, THEN MAY NEED A BRIEF PRIMER
    HERE, OR A POINTER TO THAT WITH A STATMENT THAT NEED TO UNDERSTAND THAT TO UNDERSTAND THIS.
 -- MAYBE AN EXAMPLE WOULD ALSO HELP?

 -- I do not think there is a way to understand Additive conditions without understanding the consideration queue and
    consideration sets, since adding sets to the queue and adding nodes to the sets is a central piece of what they do.
    While it may be possible to abstract this in some way, I don't see how we could do it in a way that is fundamentally
    simpler or easier to understand than just explaining the constructs directly.

    To that end, the below reference to consideration_queue now links to a newly reorganized section in the Scheduler
    docs that includes a high level description of consideration queue/consideration set construction and a graphical
    example.
    -ds

These are used to force additional executions of a Component relative to the state of the Component,
Scheduler, or context by 1) inserting the Component into sets of the `Scheduler's <Scheduler>`
`consideration_queue <Scheduler.consideration_queue>` and 2) generating a Subtractive Condition to impose additional
constraints on the execution of the Condition's owner component based on some desired state.
COMMENT

Components that are part of a collection `specified to a Scheduler for execution <Scheduler_Creation>` can be
associated with a maximum of one additive Condition and one subtractive Condition. If both an additive Condition and
a subtractive Condition are specified for a given Component, an `All <All>` Condition is generated that bundles the
Component's associated Subtractive Condition with the Subtractive Condition that is generated by the Component's
associated Additive Condition. In these cases, the bundled `All <All>` condition will be used to determine if the
Component is eligible for execution.


COMMENT:
WHAT IS A "COLLECTION" -- IS THIS A PNL CONSRUCT, OR A TERM USED FOR DESCRIPTIVE PURPOSES ONLY?

A collection is used for descriptive purposes here. To be more specific, the "collection" of components is
inferred either from a Composition or a Graph, depending on which was specified in the Scheduler's constructor.
-ds
COMMENT

.. note::
    Any Components that are part of a collection can be associated with Conditions. Most commonly, these are
...
.. _Condition_Custom_Additive:

*Additive Conditions*
=====================

Custom additive Conditions can be created by calling the constructor for the base class (`Condition()`) and assigning
ContextFlags.ADDITIVE to the condition_type argument, a function to the **func** argument, and any arguments it requires
to the **args** and/or **kwargs** arguments (for formal or keyword arguments, respectively). The function is called with
**args**, **kwargs**, and the scheduler's base consideration queue (i.e. a consideration queue that is based solely on
the structure of the `Graph <Graph>`), and should return a tuple that contains, at index 0, an iterable of indices
at which the owner component should be inserted into the consideration queue and, at index 1, a subtractive condition to
apply to the Owner component.

.. note::

    An insertion_indices iterable containing a value of -1 indicates that the final consideration queue will need
    an additional consideration set prepended to the start of the base consideration queue. Analogously, an
    insertion_indices iterable containing a value that is equal to the length of the base consideration queue indicates
    that the scheduler's final consideration queue will need an additional consideration appended to the end of the
    base consideration queue. Each of the additional sets will only be included in the final consideration queue if at
    least one Component's Additive Condition meets its respective criterion, and any Component for which this is the
    case will be included in either or both of the additional sets that it qualifies for.

.. _Condition_During_Every_Timestep_Example:

For example, the following script fragment implements an Additive Condition that forces mech_A to execute at every
timestep.

execute during every time step::

    def during_every_time_step(consideration_queue=None):
        insertion_indices = [i for i in range(len(consideration_queue))]
        condition = Always()
        return (insertion_indices, condition)
    my_scheduler.add_condition(mech_A, Condition(during_every_time_step, condition_type=ConditionType.ADDITIVE))

In the example, a function `during_every_trial` was defined that returns a tuple containing the following:
    - at index 0, an iterable that contains all of the indices of the scheduler's base consideration queue, which
    indicates that, in the final consideration queue, the owner Component should be included in every consideration set.

    - at index 1, a subtractive condition, Always(), which indicates that the Component should execute each time it is
    considered.
...
.. _Conditions_Time_Based_Subtractive:
...
.. _Conditions_Time_Based_Additive:
...
.. _Condition_Execution:

Execution
---------

.. _Condition_Execution_Subtractive:
