...
An EVCController is a `ControlMechanism <ControlMechanism>` that regulates it `ControlSignals <ControlSignal>` in
order to optimize the performance of the System to which it belongs.  EVCController is one of the most
...
structure of the EVCController.

An EVCController is similar to a standard `ControlMechanism`, with the following exceptions:
  ...
  * it has several specialized functions that are used to search over the `allocations <ControlSignal.allocations>`\\s
    ...
    <EVCController.system>`; by default, it simulates its `system <EVCController.system>` and evaluates
    its performance under all combinations of ControlSignal values to find the one that optimizes the `Expected
    Value of Control <EVCController_EVC>`, however its functions can be customized or replaced to implement
    other optimization procedures.
  ..
  * it creates a specialized set of `prediction Mechanisms` EVCController_Prediction_Mechanisms` that are used to
    simulate the performnace of its `system <EVCController.system>`.
...
The EVCController uses it `function <EVCController.function>` to select an `allocation_policy` for its
`system <EVCController.system>`.  In the `default configuration <EVCController_Default_Configuration>`,
an EVCController carries out an exhaustive evaluation of allocation policies, simulating its `system
<EVCController.system>` under each, and using an `ObjectiveMechanism` and several `auxiliary functions
<EVCController_Functions>` to calculate the **expected value of control (EVC)** for each `allocation_policy`:
a cost-benefit analysis that weighs the `cost <ControlSignal.cost> of the ControlSignals against the outcome of the
`system <EVCController.system>` \\s performance for a given `allocation_policy`. The EVCController
...
the EVCController, as described `below <EVCController_Functions>`.

.. _EVCController_Creation:

Creating an EVCController
------------------------

An EVCController can be created in any of the ways used to `create a ControlMechanism
<ControlMechanism_Creation>`; it is also created automatically when a `System` is created and the EVCController
...
EVCController can be specified in the standard way for a ControlMechanism (see
`ControlMechanism_ObjectiveMechanism` and `ControlMechanism_Control_Signals`, respectively).

.. note::
   Although an EVCController can be created on its own, it can only be assigned to, and executed within a `System`
   as the System's `controller <System.controller>`.

When an EVCController is assigned to, or created by a System, it is assigned the OutputStates to be monitored and
parameters to be controlled specified for that System (see `System_Control`), and a `prediction Mechanism
<EVCController_Prediction_Mechanisms>` is created for each `ORIGIN` Mechanism in the `system
<EVCController.system>`. The prediction Mechanisms are assigned to the EVCController's
`prediction_mechanisms` attribute. The OutputStates used to determine an EVCController’s allocation_policy and
the parameters it controls can be listed using its show method. The EVCController and the Components
associated with it in its `system <EVCController.system>` can be displayed using the System's
`System.show_graph` method with its **show_control** argument assigned as `True`

An EVCController that has been constructed automatically can be customized by assigning values to its
attributes (e.g., those described above, or its `function <EVCController.function>` as described under
`EVCController_Default_Configuration `below).


.. _EVCController_Structure:

Structure
---------

An EVCController must belong to a `System` (identified in its `system <EVCController.system>` attribute).
In addition to the standard Components of a `ControlMechanism`, has a specialized set of `prediction mechanisms
<EVCController_Prediction_Mechanisms>` and `functions <EVCController_Functions>` that it uses to simulate
and evaluate the performance of its `system <EVCController.system>` under the influence of different values of
its `ControlSignals <EVCController_ControlSignals>`.  Each of these specialized Components is described below.

.. _EVCController_Input:

*Input*
~~~~~~~

.. _EVCController_ObjectiveMechanism:

ObjectiveMechanism
^^^^^^^^^^^^^^^^^^

Like any ControlMechanism, an EVCController receives its input from the *OUTCOME* `OutputState
<ObjectiveMechanism_Output>` of an `ObjectiveMechanism`, via a MappingProjection to its `primary InputState
<InputState_Primary>`.  The ObjectiveFunction is listed in the EVCController's `objective_mechanism
<EVCController.objective_mechanism>` attribute.  By default, the ObjectiveMechanism's function is a
...
in a variety of ways:
    ...
    * using a list to specify the OutputStates to be monitored  (and the `tuples format
      ...
      **objective_mechanism** argument of the EVCController's constructor;
    ..
    * using the  **monitored_output_states** argument for an ObjectiveMechanism specified in the `objective_mechanism
      <EVCController.objective_mechanism>` argument of the EVCController's constructor;
    ..
    * specifying a different `ObjectiveMechanism` in the **objective_mechanism** argument of the EVCController's
      constructor. The result of the `objective_mechanism <EVCController.objective_mechanism>`'s `function
      <ObjectiveMechanism.function>` is used as the outcome in the calculations described below.

    .. _EVCController_Objective_Mechanism_Function_Note:

    .. note::
       If a constructor for an `ObjectiveMechanism` is used for the **objective_mechanism** argument of the
       EVCController's constructor, then the default values of its attributes override any used by the
       EVCController for its `objective_mechanism <EVCController.objective_mechanism>`.  In particular,
       whereas an EVCController uses the same default `function <ObjectiveMechanism.function>` as an
       ...
       an ObjectiveMechanism is used to specify the EVCController's **objective_mechanism** argument,
...
The result of the EVCController's `objective_mechanism <EVCController.objective_mechanism>` is used by
its `function <ObjectiveMechanism.function>` to evaluate the performance of its `system <EVCController.system>`
when computing the `EVC <EVCController_EVC>`.


.. _EVCController_Prediction_Mechanisms:

Prediction Mechanisms
^^^^^^^^^^^^^^^^^^^^^

These are used to provide input to the `system <EVCController.system>` if the EVCController's
`function <EVCController.function>` (`ControlSignalGridSearch2`) `simulates its execution
<EVCController_Default_Configuration>` to evaluate the EVC for a given `allocation_policy`.  When an
EVCController is created, a prediction Mechanism is created for each `ORIGIN` Mechanism in its `system
<EVCController.system>`, and are listed in the EVCController's `prediction_mechanisms
<EVCController.prediction_mechanisms>` attribute in the same order as the `ORIGIN` Mechanisms are
listed in the `system <EVCController.system>`\\'s `origin_mechanisms <System.origin_mechanisms>` attribute.
...
EVCController's constructor, which can be specified using any of the following formats:

  * **Mechanism** -- convenience format for cases in which the EVCController's `system
    <EVCController.system>` has a single `ORIGIN` Mechanism;  the Mechanism must have the same number of
    `InputStates <InputState>` as the `system <EVCController.system>`\\'s `ORIGIN` Mechanism, and
  ...
  * **2-item tuple:** *(Mechanism subclass, dict)* -- the Mechanism subclass, and parameters specified in
  ...
  ..
  * **list** -- its length must equal the number of `ORIGIN` Mechanisms in the EVCController's `system
    <EVCController.system>` each item must be a Mechanism, a subclass of one, or a 2-item tuple (see above),
...
The prediction mechanisms for an EVCController are listed in its `prediction_mechanisms` attribute.

.. _EVCController_Functions:

*Function*
~~~~~~~~~~

By default, the primary `function <EVCController.function>` is `ControlSignalGridSearch2` (see
`EVCController_Default_Configuration`), that systematically evaluates the effects of its ControlSignals on the
performance of its `system <EVCController.system>` to identify an `allocation_policy
<EVCController.allocation_policy>` that yields the highest `EVC <EVCController_EVC>`.  However,
any function can be used that returns an appropriate value (i.e., that specifies an `allocation_policy` for the
number of `ControlSignals <EVCController_ControlSignals>` in the EVCController's `control_signals`
attribute, using the correct format for the `allocation <ControlSignal.allocation>` value of each ControlSignal).
In addition to its primary `function <EVCController.function>`, an EVCController has several auxiliary
functions, that can be used by its `function <EVCController.function>` to calculate the EVC to select an
`allocation_policy` with the maximum EVC among a range of policies specified by its ControlSignals.  The default
set of functions and their operation are described in the section that follows;  however, the EVCController's
`function <EVCController.function>` can call any other function to customize how the EVC is calcualted.

.. _EVCController_Default_Configuration:

Default Configuration of EVC Function and its Auxiliary Functions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In its default configuration, an EVCController simulates and evaluates the performance of its `system
<EVCController.system>` under a set of allocation_policies determined by the `allocation_samples
<ControlSignal.allocation_samples>` attributes of its `ControlSignals <EVCController_ControlSignals>`, and
implements (for the next `TRIAL` of execution) the one that generates the maximum `EVC <EVCController_EVC>`.
This is carried out by the EVCController's default `function <EVCController.function>` and three
auxiliary functions, as described below.

The default `function <EVCController.function>` of an EVCController is `ControlSignalGridSearch2`. It
identifies the `allocation_policy` with the maximum `EVC <EVCController_EVC>` by a conducting an exhaustive
search over every possible `allocation_policy`— that is, all combinations of `allocation <ControlSignal.allocation>`
values for its `ControlSignals <EVCController_ControlSignals>`, where the `allocation
<ControlSignal.allocation>` values sampled for each ControlSignal are determined by its `allocation_samples`
attribute.  For each `allocation_policy`, the EVCController executes the `system <EVCController.system>`,
evaluates the `EVC <EVCController_EVC>` for that policy, and returns the `allocation_policy` that yields the
greatest EVC value. The following steps are used to calculate the EVC for each `allocation_policy`:
  ...
  * **Implement the policy and simulate the System** - assign the `allocation <ControlSignal.allocation>` that the
    ...
    <EVCController.system>` using the corresponding parameter values by calling the System's `run_simulation
    <System.run_simulation>` method; this uses the `value <PredictionMechanism.value>` of eacah of the
    EVCController's `prediction_mechanisms <EVCController.prediction_mechanisms>` as input to the
    corresponding `ORIGIN` Mechanisms of its `system <EVCController.system>` (see `PredictionMechanism`).  The
    values of all :ref:`stateful attributes` of 'Components` in the System are :ref:`re-initialized` to the same value
    prior to each simulation, so that the results for each `allocation_policy <EVCController.allocation_policy>`
    are based on the same initial conditions.  Each simulation includes execution of the EVCController's
    `objective_mechanism`, which provides the result to the EVCController.  If `system
    <EVCController.system>`\\.recordSimulationPref is `True`, the results of each simulation are appended to the
    `simulation_results <System.simulation_results>` attribute of `system <EVCController.system>`.
  |
  * **Evaluate the System's performance** - this is carried out by the EVCController's `objective_mechanism
    <EVCController.objective_mechanism>`, which is executed as part of the simulation of the System.  The
    `function <ObjectiveMechanism.function>` for a default ObjectiveMechanism is a `LinearCombination` Function that
    combines the `value <OutputState.value>`\\s of the OutputStates listed in the EVCController's
    `monitored_output_states <EVCController.monitored_output_states>` attribute (and the `objective_mechanism
    <EVCController.objective_mechanism>`'s `monitored_output_states <ObjectiveMechanism.monitored_output_states>`
    attribute) by taking their elementwise (Hadamard) product.  However, this behavior can be customized in a variety
    of ways, as described `above <EVCController_ObjectiveMechanism>`.
  |
  * **Calculate EVC** - call the EVCController's `value_function <EVCController.value_function>` passing it
    the outcome (received from the `objective_mechanism`) and a list of the `costs <ControlSignal.cost>` \\s of its
    `ControlSignals <EVCController_ControlSignals>`.  The default `value_function
    <EVCController.value_function>` calls two additional auxiliary functions, in the following order:
    |
    - `cost_function <EVCController.cost_function>`, which sums the costs;  this can be configured to weight
      and/or exponentiate individual costs (see `cost_function <EVCController.cost_function>` attribute);
    |
    - `combine_outcome_and_cost_function <EVCController.combine_outcome_and_cost_function>`, which subtracts the
      sum of the costs from the outcome to generate the EVC;  this too can be configured (see
      `combine_outcome_and_cost_function <EVCController.combine_outcome_and_cost_function>`).

In addition to modifying the default functions (as noted above), any or all of them can be replaced with a custom
function to modify how the `allocation_policy <EVCController.allocation_policy>` is determined, so long as the
...
.. _EVCController_Calling_and_Assigning_Functions:

    .. note::
       The `EVCController auxiliary functions <EVCController_Functions>` described above are all
       implemented as PsyNeuLink `Functions <Function>`.  Therefore, to call a function itself, it must be referenced
       as ``<EVCController>.<function_attribute>.function``.  A custom function assigned to one of the auxiliary
       ...
       ``<EVCController>.<function_attribute>.function``.

.. _EVCController_ControlSignals:

*ControlSignals*
~~~~~~~~~~~~~~~~

The OutputStates of an EVCController (like any `ControlMechanism`) are a set of `ControlSignals
<ControlSignal>`, that are listed in its `control_signals <EVCController.control_signals>` attribute (as well as
its `output_states <ControlMechanism.output_states>` attribute).  Each ControlSignal is assigned a  `ControlProjection`
that projects to the `ParameterState` for a parameter controlled by the EVCController.  Each ControlSignal is
assigned an item of the EVCController's `allocation_policy`, that determines its `allocation
...
and/or its time course. The `cost <ControlSignal.cost>` is included in the evaluation that the EVCController
carries out for a given `allocation_policy`, and that it uses to adapt the ControlSignal's `allocation
<ControlSignal.allocation>` in the future.  When the EVCController chooses an `allocation_policy` to evaluate,
it selects an allocation value from the ControlSignal's `allocation_samples <ControlSignal.allocation_samples>`
attribute.


.. _EVCController_Execution:

Execution
---------

An EVCController must be the `controller <System.controller>` of a System, and as a consequence it is always the
last `Mechanism <Mechanism>` to be executed in a `TRIAL` for its `system <EVCController.system>` (see `System
Control <System_Execution_Control>` and `Execution <System_Execution>`). When an EVCController is executed,
it updates the value of its `prediction_mechanisms` and `objective_mechanism`, and then calls its `function
<EVCController.function>`, which determines and implements the `allocation_policy` for the next `TRIAL` of its
`system <EVCController.system>`\\s execution.  The default `function <EVCController.function>` executes
the following steps (described in greater detailed `above <EVCController_Default_Configuration>`):

* Samples every allocation_policy (i.e., every combination of the `allocation` \\s specified for the
  EVCController's ControlSignals specified by their `allocation_samples` attributes);  for each
  `allocation_policy`, it:

  * Runs a simulation of the EVCController's `system <EVCController.system>` with the parameter values
    specified by that `allocation_policy`, by calling the system's  `run_simulation <System.run_simulation>` method;
    each simulation uses inputs provided by the EVCController's `prediction_mechanisms
    <EVCController.prediction_mechanisms>` and includes execution of its `objective_mechanism`, which provides
    its result to the EVCController.

  * Calls the EVCController's `value_function <EVCController.value_function>`, which in turn calls
    EVCController's `cost_function <EVCController.cost_function>` and `combine_outcome_and_cost_function
    <EVCController.combine_outcome_and_cost_function>` to evaluate the EVC for that `allocation_policy`.
...
<EVCController_Functions>` referred to above.


.. _EVCController_Examples:

Example
-------

The following example implements a System with an EVCController (and two processes not shown)::
    ...
    ...                       controller=pnl.EVCController,                       #doctest: +SKIP
...
.. _EVCController_Class_Reference:
...
class EVCController(Controller):
    """EVCController(                                            \
    system=True,                                                       \
    ...
    COMMENT:
       ...
       **********************************************************************************************
       ...
       NOT CURRENTLY IN USE:
        ...
        system : System
            System for which the EVCController is the controller;  this is a required parameter.
    ...
    system : System : default None
        specifies the `System` for which the EVCController should serve as a `controller <System.controller>`;
        the EVCController will inherit any `OutputStates <OutputState>` specified in the **monitor_for_control**
        argument of the `system <EVCController.system>`'s constructor, and any `ControlSignals <ControlSignal>`
    ...
    default MonitoredOutputStatesOptions.PRIMARY_OUTPUT_STATES
        specifies either an `ObjectiveMechanism` to use for the EVCController or a list of the OutputStates it should
    ...
    default PredictionMechanism
        ...
        <EVCController_Prediction_Mechanisms>` and, optionally, their parameters (specified in a `parameter
        specification dictionary <ParameterState_Specification>`);  see `EVCController_Prediction_Mechanisms`
        for details.

        COMMENT:
        the `Mechanism(s) <Mechanism>` or class(es) of Mechanisms  used for `prediction Mechanism(s)
        <EVCController_Prediction_Mechanisms>`.  If a class, dict, or tuple is specified, it is used as the
        ...
        the number of `ORIGIN` Mechanisms in the System for which the EVCController is the `controller
        ...
        ..note::
            ...
            the System for which the EVCController is the `controller <System.controller>` has a single `ORIGIN`
    ...
    function : function or method : ControlSignalGridSearch2
        ...
        EVCController's `system <EVCController.system>` (see `function <EVCController.function>` for details).

    value_function : function or method : value_function
        specifies the function used to calculate the `EVC <EVCController_EVC>` for the current `allocation_policy`
        (see `value_function <EVCController.value_function>` for details).
    ...
    cost_function : function or method : LinearCombination(operation=SUM)
        ...
        (see `cost_function <EVCController.cost_function>` for details).
    ...
    control_signals : ControlSignal specification or List[ControlSignal specification, ...]
        specifies the parameters to be controlled by the EVCController
        (see `ControlSignal_Specification` for details of specification).

    params : Dict[param keyword: param value] : default None
        a `parameter dictionary <ParameterState_Specification>` that can be used to specify the parameters for the
        Mechanism, its `function <EVCController.function>`, and/or a custom function and its parameters.  Values
        specified for parameters in the dictionary override any assigned to those parameters in arguments of the
        constructor.

    name : str : default see `name <EVCController.name>`
        specifies the name of the EVCController.

    prefs : PreferenceSet or specification dict : default Mechanism.classPreferences
        specifies the `PreferenceSet` for the EVCController; see `prefs <EVCController.prefs>` for details.

    Attributes
    ----------

    system : System
        the `System` for which EVCController is the `controller <System.controller>`;
        the EVCController inherits any `OutputStates <OutputState>` specified in the **monitor_for_control**
        argument of the `system <EVCController.system>`'s constructor, and any `ControlSignals <ControlSignal>`
        specified in its **control_signals** argument.

    prediction_mechanisms : List[ProcessingMechanism]
        list of `predictions mechanisms <EVCController_Prediction_Mechanisms>` generated for the
        EVCController's `system <EVCController.system>` when the EVCController is created,
        one for each `ORIGIN` Mechanism in the `system <EVCController.system>`.  Each prediction Mechanism is
    ...
    origin_prediction_mechanisms : Dict[ProcessingMechanism, ProcessingMechanism]
        dictionary of `prediction mechanisms <EVCController_Prediction_Mechanisms>` added to the EVCController's
        `system <EVCController.system>`, one for each of its `ORIGIN` Mechanisms.  The key for each
    ...
    predicted_input : Dict[ProcessingMechanism, value]
        ...
        <EVCController_Prediction_Mechanisms>` listed in `prediction_mechanisms` corresponding to each `ORIGIN`
    ...
    objective_mechanism : ObjectiveMechanism
        the 'ObjectiveMechanism' used by the EVCController to evaluate the performance of its `system
        <EVCController.system>`.  If a list of OutputStates is specified in the **objective_mechanism** argument of the
        EVCController's constructor, they are assigned as the `monitored_output_states <ObjectiveMechanism.monitored_output_states>`
        attribute for the `objective_mechanism <EVCController.objective_mechanism>`.

    monitored_output_states : List[OutputState]
        list of the OutputStates monitored by `objective_mechanism <EVCController.objective_mechanism>` (and listed in
        its `monitored_output_states <ObjectiveMechanism.monitored_output_states>` attribute), and used to evaluate the
        performance of the EVCController's `system <EVCController.system>`.
    ...
    [TBI]
        monitored_output_states : 3D np.array
            ...
            the EVCController's `input_states <EVCController.input_states>`).
    COMMENT
    ...
    function : function : default ControlSignalGridSearch2
        ...
        the greatest `EVC <EVCController_EVC>` value (see `EVCController_Default_Configuration` for additional details).
        If a custom function is specified, it must accommodate a **controller** argument that specifies an EVCController
        (and provides access to its attributes, including `control_signal_search_space`), and must return an array with
        the same format (number and type of elements) as the EVCController's `allocation_policy` attribute.
    ...
    value_function : function : default ValueFunction2
        calculates the `EVC <EVCController_EVC>` for a given `allocation_policy`.  It takes as its arguments an
        `EVCController`, an **outcome** value and a list or ndarray of **costs**, uses these to calculate an EVC,
        and returns a three item tuple with the calculated EVC, and the outcome value and aggregated value of costs
        used to calculate the EVC.  The default, `ValueFunction2`,  calls the EVCController's `cost_function
        <EVCController.cost_function>` to aggregate the value of the costs, and then calls its
        `combine_outcome_and_costs <EVCController.combine_outcome_and_costs>` to calculate the EVC from the outcome
        and aggregated cost (see `EVCController_Default_Configuration` for additional details).  A custom
        ...
        values or `None`). If used with the EVCController's default `function <EVCController.function>`, a custom
        `value_function` must accommodate three arguments (passed by name): a **controller** argument that is the
        EVCController for which it is carrying out the calculation; an **outcome** argument that is a value; and a
        `costs` argument that is a list or ndarray.  A custom function assigned to `value_function` can also call any
        of the `helper functions <EVCController_Functions>` that it calls (however, see `note
        <EVCController_Calling_and_Assigning_Functions>` above).
    ...
    cost_function : function : default LinearCombination(operation=SUM)
        ...
        function sums the `cost <ControlSignal.cost>` of each of the EVCController's `ControlSignals
        <EVCController_ControlSignals>`.  The `weights <LinearCombination.weights>` and/or `exponents
        ...
        EVCController's `control_signals <EVCController.control_signals>` attribute. The default function can also be
        replaced with any `custom function <EVCController_Calling_and_Assigning_Functions>` that takes an array as
        input and returns a scalar value.  If used with the EVCController's default `value_function
        <EVCController.value_function>`, a custom `cost_function <EVCController.cost_function>` must accommodate two
        arguments (passed by name): a **controller** argument that is the EVCController itself;  and a **costs**
    ...
    combine_outcome_and_cost_function : function : default LinearCombination(operation=SUM)
        combines the outcome and cost for given `allocation_policy` to determine its `EVC <EVCControllers_EVC>`. The
        ...
        function, as described for the `cost_function <EVCControllers.cost_function>`.  The default function can also be
        replaced with any `custom function <EVCController_Calling_and_Assigning_Functions>` that returns a scalar value.  If used with the EVCController's default `value_function`, a custom
        If used with the EVCController's default `value_function`, a custom combine_outcome_and_cost_function must
        accomoudate three arguments (passed by name): a **controller** argument that is the EVCController itself; an
    ...
    control_signal_search_space : 2d np.array
        ...
        specified for each by its `allocation_samples <EVCController.allocation_samples>` attribute.

    EVC_max : 1d np.array with single value
        the maximum `EVC <EVCController_EVC>` value over all allocation policies in `control_signal_search_space`.
    ...
    EVC_policies : 2d np.array
        array with every `allocation_policy` tested in `control_signal_search_space`.  The `EVC <EVCController_EVC>`
        value of each is stored in `EVC_values`.

    EVC_values :  1d np.array
        array of `EVC <EVCController_EVC>` values, each of which corresponds to an `allocation_policy` in `EVC_policies`;
    ...
    allocation_policy : 2d np.array : defaultControlAllocation
        ...
        number of ControlSignals in the EVCController's `control_signals` attribute.

    control_signals : ContentAddressableList[ControlSignal]
        list of the EVCController's `ControlSignals <EVCController_ControlSignals>`, including any that it inherited
        from its `system <EVCController.system>` (same as the EVCController's `output_states
        <Mechanism_Base.output_states>` attribute); each sends a `ControlProjection` to the `ParameterState` for the
        parameter it controls

    name : str
        the name of the EVCController; if it is not specified in the **name** argument of the constructor, a
        default is assigned by MechanismRegistry (see `Naming` for conventions used for default and duplicate names).

    prefs : PreferenceSet or specification dict
        the `PreferenceSet` for the EVCController; if it is not specified in the **prefs** argument of the
