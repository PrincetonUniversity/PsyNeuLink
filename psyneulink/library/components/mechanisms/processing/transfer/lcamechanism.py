...
The *noise*, *leak*, *initial_value*, and *time_step_size* arguments are used to implement the `LCAIntegratorFunction`
...
1. its `integrator_function <LCAMechanism.integrator_function>`, which implements the `LCAIntegratorFunction`. (Note that a
...
from psyneulink.core.components.functions.statefulfunctions.integratorfunctions import LCAIntegratorFunction
...
class LCAMechanism(RecurrentTransferMechanism):
    ...
    1. its `integrator_function <LCAMechanism.integrator_function>`, which implements the `LCAIntegratorFunction`. (where *rate*
    = *leak*)
    ...
    leak : value : default 0.5
        sets the `rate <LCAIntegratorFunction.rate>` on the `LCAIntegratorFunction function <LCAIntegratorFunction>`, which scales the
        contribution of the `LCAIntegratorFunction's <LCAIntegratorFunction>` `previous_value <LCAIntegratorFunction.previous_value>` to the
        accumulation of the `LCAIntegratorFunction's value <LCAIntegratorFunction.value>` (:math:`x_{i}`) on each time step. See
        `LCAIntegratorFunction <LCAIntegratorFunction>` for more details on what the `LCAIntegratorFunction function  <LCAIntegratorFunction>` computes.
    ...
    competition : value : default 1.0
        ...
        `LCAIntegratorFunction's value <LCAIntegratorFunction.value>` (:math:`x_{i}`) on each time step. See
        `LCAIntegratorFunction <LCAIntegratorFunction>` for more details on what the `LCAIntegratorFunction function  <LCAIntegratorFunction>` computes.
    ...
    self_excitation : value : default 0.0
        ...
        recurrent value (:math:`f(x)_{i}`) to the accumulation of the `LCAIntegratorFunction's value <LCAIntegratorFunction.value>`
        (:math:`x_{i}`) on each time step. See `LCAIntegratorFunction <LCAIntegratorFunction>` for more details on what the
        `LCAIntegratorFunction function  <LCAIntegratorFunction>` computes.
    ...
    integrator_function:
        ...
        which is the `LCAIntegratorFunction`. See `LCAIntegratorFunction <LCAIntegratorFunction>` for more details on what it computes.
        Keep in mind that the `leak <LCAMechanism.leak>` parameter of the `LCAMechanism` determines the `rate <LCAIntegratorFunction.rate>` of the
    ...
    def __init__(self,
        ...
        integrator_function = LCAIntegratorFunction
    ...
    def _get_integrated_function_input(self, function_variable, initial_value, noise, context, execution_id=None):
        ...
        if self.context.initialization_status == ContextFlags.INITIALIZING:
            self.integrator_function = LCAIntegratorFunction(
                function_variable,
