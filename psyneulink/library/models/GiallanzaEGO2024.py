...
INTEGRATION_RATE = .5  # The integration rate fot the context representation. In the original model, this was an adaptive gate, see note above.
SOFTMAX_THRESHOLD = 1e-3  # The softmax threshold for memory retrieval. All retrievals with a probability below this threshold are ignored.
...
def construct_model(
    ...
    retrieval_softmax_threshold: float = SOFTMAX_THRESHOLD
    state_retrieval_weight: Optional[float] = STATE_RETRIEVAL_WEIGHT
    previous_state_retrieval_weight: Optional[float] = PREVIOUS_STATE_RETRIEVAL_WEIGHT
    context_retrieval_weight: Optional[float] = CONTEXT_RETRIEVAL_WEIGHT
    normalize_field_weights: bool = NORMALIZE_FIELD_WEIGHT
    normalize_memories: bool = NORMALIZE_MEMORIES
    concatenate_queries: bool = CONCATENATE_QUERIES
    ...
    previous_state_layer = pnl.ProcessingMechanism(name="PREVIOUS STATE", input_shapes=STATE_SIZE)
...
def run_model(model,
    ...
    model.learn(inputs={input_layer: trials},
...
def main():
    ...
    results_blocked = run_model(model_blocked, input_blocked, states_blocked_one_hot)
    # We can plot the models accuarcy by comparing the predictions to the target states
    ...
    results_interleaved = run_model(model_interleaved, input_interleaved, states_interleaved_one_hot)
    if STORE_FIG or SHOW_FIG:
...
# # example
# if __name__ == "__main__":
#     X, Y = generate_io(paradigm="blocked", probs=[1.,1.,1.], n=3, seed=42)
#     print(X.shape, Y.shape)  # (T-1, 11) (T-1, 11)
#
#
#
# if __name__ == '__main__':
#     trials = [[1, 0, 0, 0, 0], [0, 1, 0, 1, 0]]
#     model = construct_model(memory_capacity=5, state_size=5, context_size=5)
