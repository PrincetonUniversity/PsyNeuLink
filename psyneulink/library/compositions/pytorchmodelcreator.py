...
class PytorchModelCreator(torch.nn.Module):
            ...
            for component in current_exec_set:
                ...
                if i == 0:
                    ...
                    for j, (input_vertex, weights) in enumerate(afferents.items()):
                        ...
                        pnlvm.helpers.inject_printf_float_matrix(builder, weights_llvmlite, prefix=f"{source_node} -> {component}\tweight:\t")
                        # node inputs are 2d arrays in a struct
                ...
                if store_z_values is True:
                    pnlvm.helpers.inject_printf_float_array(builder, z_values[component], prefix=f"{component}\tforward input:\t")
                pnlvm.helpers.inject_printf_float_array(builder, value, prefix=f"{component}\tforward output:\t")
    ...
    def _gen_llvm_training_backprop(self, ctx, optimizer, loss):
        ...
        for i, node in enumerate(input_nodes):
            ...
            pnlvm.helpers.inject_printf_float_array(builder, node_model_input, prefix=f"{node}\tinput:\t")
        ...
        while(len(backprop_queue) > 0):
            ...
            if node in output_nodes:
                ...
                pnlvm.helpers.inject_printf_float_array(builder, node_target, prefix=f"{node}\ttarget:\t")
                pnlvm.helpers.inject_printf_float_array(builder, node_output, prefix=f"{node}\tvalue:\t")

                pnlvm.helpers.inject_printf(builder,f"{node}\tloss:\t%f\n",tmp_loss)
            ...
            pnlvm.helpers.inject_printf_float_array(builder, activation_func_derivative, prefix=f"{node}\tdSigma:\t")
            pnlvm.helpers.inject_printf_float_array(builder, error_val, prefix=f"{node}\terror:\t")
        ...
        pnlvm.helpers.inject_printf(builder,"TOTAL LOSS:\t%f\n",builder.load(total_loss))
        builder.ret_void()
    ...
    def _gen_llvm_training_function_body(self, ctx, builder, context, params, data, autodiff_stimuli_struct):
        ...
        pnlvm.helpers.inject_printf(builder,"Running Autodiff Training with params:\n\tepoch count: %d \n\tnum_trials: %d \n",
                            epochs,
                            num_trials)

        pnlvm.helpers.inject_printf(builder,"\tlearning_struct_addr: 0x%Lx \n",
                            training_set_array)
        ...
        if optimizer_type == 'adam':
            pnlvm.helpers.inject_printf(builder,"Running with ADAM optimizer\n")
            optimizer = AdamOptimizer(self,lr = self._composition.learning_rate)
        elif optimizer_type == 'sgd':
            pnlvm.helpers.inject_printf(builder,"Running with SGD optimizer\n")
        ...
        with pnlvm.helpers.for_loop_zero_inc(builder, epochs, "epoch_loop") as (b1, epoch_idx):
            pnlvm.helpers.inject_printf(builder, "\033[0;32mEPOCH %d\033[0m\n", epoch_idx)
            with pnlvm.helpers.for_loop_zero_inc(b1, num_trials, "input_loop") as (b2, trial_num):
                pnlvm.helpers.inject_printf(b2, "\n\033[0;31mINPUT %d\033[0m\n", trial_num)
                pnlvm.helpers.inject_printf(b2, "OPTIMIZER ZERO GRAD %d\n", trial_num)
                ...
                pnlvm.helpers.inject_printf(b2, "TRIAL %d\n", trial_num)
                b2.call(backprop, [context, params, model_input, data,
                                   optimizer_struct, training_set_array,
                                   trial_num])
                pnlvm.helpers.inject_printf(b2, "OPTIMIZER STEP %d\n", trial_num)
