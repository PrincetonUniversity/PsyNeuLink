...
class PytorchModelCreator(torch.nn.Module):
    ...
    def __init__(self, composition, device, context=None):
        ...
        self.execution_sets = [{self.node_map[comp] for comp in s if comp in self.node_map} for s in self.execution_sets]
    ...
    def _get_afferent_node_index(self, node, afferent_node):
        return [proj.receiver for proj in node.afferents].index(self.node_map[afferent_node])

    def _get_afferent_nodes(self, node):
        forward_info_weights = self.node_map[node].afferents
