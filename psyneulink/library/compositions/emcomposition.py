        ...
        """
            ...
            ----------
                ...
                storage_prob
                    ...
                    :type: ``float``
        """
        memory = Parameter(None, loggable=True, getter=_memory_getter, read_only=True, pnl_internal=True)
        memory_template = Parameter([[0],[0]], structural=True, valid_types=(tuple, list, np.ndarray), read_only=True, pnl_internal=True)
        memory_capacity = Parameter(1000, structural=True, pnl_internal=True)
        field_weights = Parameter(None, pnl_internal=True)
        normalize_field_weights = Parameter(True, pnl_internal=True)
        field_names = Parameter(None, structural=True, pnl_internal=True)
        concatenate_queries = Parameter(False, structural=True, pnl_internal=True)
        normalize_memories = Parameter(True, pnl_internal=True)
        softmax_gain = Parameter(1.0, modulable=True, pnl_internal=True)
        softmax_threshold = Parameter(.001, modulable=True, specify_none=True, pnl_internal=True)
        softmax_choice = Parameter(WEIGHTED_AVG, modulable=False, specify_none=True, pnl_internal=True)
        storage_prob = Parameter(1.0, modulable=True, aliases=[MULTIPLICATIVE_PARAM],pnl_internal=True)
        memory_decay_rate = Parameter(AUTO, modulable=True,pnl_internal=True)
        enable_learning = Parameter(True, structural=True, pnl_internal=True)
        learn_field_weights = Parameter(True, structural=True, pnl_internal=True)
        learning_rate = Parameter(.001, modulable=True, pnl_internal=True)
        random_state = Parameter(None, loggable=False, getter=_random_state_getter, dependencies='seed')
        seed = Parameter(DEFAULT_SEED(), modulable=True, setter=_seed_setter, pnl_internal=True)
