...
# END
    ...
    *show_pytorch* argument to True (see `PytorchShowGraph` for additional information).
    ...
    def _parse_learning_rate_specs(self,
        ...
        #                        for proj in [p.projection for p in self.projection_wrappers
        #                                     if LEARNING in p._use and p.projection.learnable]}
        # MODIFIED 8/5/25 NEWER:
        # projection_lr_specs = {proj.name:TorchParamTuple(proj, proj.parameters.learning_rate.get(context))
        #                        for proj in [p.projection for p in self.projection_wrappers
        #                                     if LEARNING in p._use and p.projection.learnable and
        #                                     is_numeric_scalar(p.projection.parameters.learning_rate.get(context))]}
        # MODIFIED 8/5/25 END
    ...
    def _assign_learning_rates(self,
        ...
        for old_param_group, new_param_group in zip(old_param_groups, new_param_groups):
            ...
            for param in old_param_group['params']:
                ...
                # # MODIFIED 8/5/25 OLD:
                # specified_learning_rate = (optimizer_torch_params_full_with_specified[param]
                #                            if optimizer_torch_params_full_with_specified
                #                               and param in optimizer_torch_params_full_with_specified
                #                            else specified_learning_rate)
                # MODIFIED 8/5/25 NEW:
                # BREADCRUMB: COMPLETE COMMENT BELOW
                # If learning rate is specified for a projection in call to learn(), that takes precedence over...
                lr_specified_in_learn = False
                if source == LEARN_OVERRIDE and optimizer_user_parsed:
                    if projection.name in optimizer_user_parsed:
                        specified_learning_rate = optimizer_user_parsed[projection.name].value
                        lr_specified_in_learn = True
                    elif hasattr(projection, PROXY_FOR_ATTRIB) and projection._proxy_for.name in optimizer_user_parsed:
                        specified_learning_rate = optimizer_user_parsed[projection._proxy_for.name].value
                        lr_specified_in_learn = True
                # MODIFIED 8/5/25 END
                # # MODIFIED 8/4/25 NEW:
                # # Get learning_rate specified by the user (in learn(), or constructor for Compositon or Projection):
                ...
                # # Get learning_rate specified by the user (in learn(), or constructor for Compositon or Projection):
                # if optimizer_torch_params_full_with_specified and param in optimizer_torch_params_full_with_specified:
                #     specified_learning_rate = optimizer_torch_params_full_with_specified[param]
                ...
                if proj_composition.enable_learning is False or projection.learnable is False:
                    ...
                    if specified_learning_rate in {True, None}:
                        ...
                        # MODIFIED 8/4/25 NEW:
                        # BREADCRUMB: IF A PROJECTION IS SPECIFIED AS NONE IN learn(), THAT SHOULD BE ASSIGNED
                        #             TO proj_default_lr HERE, AND THEN USED IN THE ASSIGNMENT BELOW
                        ...
                        if proj_comp_lr is False or comp_lr is False:
                            ...
                            # Use either run_time learning_rate or learning_rate for Composition, giving precedence
                            #   to one to which the Projection belongs if it is in a nested Composition
                            # BREADCRUMB: COMMENTING OUT proj_default_lr CAUSES FAIL IN test_learn_rate_with_3_levels...
                            #             BUT LEAVING IT IN CAUSES FAILS IN test_learning_rate_inheritance...
                            specified_learning_rate = \
    ...
    def _restore_constructor_proj_learning_rates_and_torch_params(self, optimizer:torch.optim.Optimizer, context):
        ...
        #     comp_constructor_learning_rate = self.composition.parameters.learning_rate.get(None)
        #     self.composition.parameters.learning_rate.set(comp_constructor_learning_rate, context)
        # # # MODIFIED 7/30/25 NEWEST:
        # try:
        #     # BREADCRUMB:  NEED TO MODIFY ASSERTS BELOW TO USE self.composition._constructor_param_groups
        #     self.optimizer.param_groups = self._copy_torch_param_groups(self.composition._constructor_param_groups)
        #     for proj in self.wrapped_projections:
        #         proj.parameters.learning_rate.set(self._constructor_proj_learning_rates[proj], context)
        #     comp_constructor_learning_rate = self.composition.parameters.learning_rate.get(None)
