...
--------
  ...
  * `PyTorchCFA_Overview`
  * `PyTorchCFA_Class_Reference`


.. _PyTorchCFA_Overview:

Overview
--------

A `PyTorchCFA` is a subclass of `CompositionFunctionApproximator` that uses PyTorch to parameterize (learn) the `matrix
<MappingProjection.matrix>` Parameters of Projections in an AutodiffComposition over trials to predict the `net_outcome
<ControlMechanism.net_outcome>` for a `Composition` (or part of one) controlled by an `OptimizationControlMechanism`.
The matrix Parameters are updated by the `update_weights <PyTorchCFA.update_weights>` method, that is the
autodiff_backward() method of the AutodiffComposition, the `learn <Composition.learn>` method of which is assigned as
the PyTorchCFA's `adapt <CompositionFunctionApproximator.adapt>` method, and the `run <Composition.run>` method of
which is called by the `evaluate <CompositionFunctionApproximator.evaluate>` method to predict the `net_outcome
<ControlMechanism.net_outcome>` for a `Composition` (or part of one) controlled by an `OptimizationControlMechanism`,
based on a set of `state_feature_values <OptimizationControlMechanism.state_feature_values>`, a `control_allocation
<ControlMechanism.control_allocation>` (if specified), and the `net_outcome <ControlMechanism.net_outcome>` they
produce, passed to it from an `OptimizationControlMechanism`.

COMMENT:
Its `evaluate <CompositionFunctionApproximator.evaluate>` method calls its `update_weights
<PyTorchCFA.regresssion_function>` to generate and return a predicted `net_outcome
...
<ControlMechanism.control_allocation>` passed to the PyTorchCFA's `adapt <PyTorchCFA.adapt>` method, and provided
as the input to its `update_weights <PyTorchCFA.update_weights>`, are represented in the `vector
<PredictionVector.vector>` attribute of a `PredictionVector` assigned to the PyTorchCFA`s `prediction_vector
<PyTorchCFA.prediction_vector>` attribute.  The  `state_feature_values
<OptimizationControlMechanism.state_feature_values>` are assigned to the state_features field of the
`prediction_vector <PyTorchCFA.prediction_vector>`, and the `control_allocation
<ControlMechanism_control_allocation>` is assigned to the control_allocation field of the `prediction_vector
<PyTorchCFA.prediction_vector>`.  The `prediction_vector <PyTorchCFA.prediction_vector>` may also contain fields
...
The `regression_weights <PyTorchCFA.regression_weights>` returned by the `update_weights
<PyTorchCFA.update_weights>` are used by the PyTorchCFA's `evaluate <PyTorchCFA.evaluate>` method to predict
the `net_outcome <ControlMechanism.net_outcome>` from the `prediction_vector <PyTorchCFA.prediction_vector>`.


COMMENT:
.. note::
  The PyTorchCFA's `update_weights <PyTorchCFA.update_weights>` is provided the `state_feature_values
...
__all__ = ['PREDICTION_TERMS', 'PV', 'PyTorchCFA']
...
# class PV(AutoNumberEnum):
    ...
    F
        Main effect of `state_feature_values <OptimizationControlMechanism_Feature_values>`.
    C
        Main effect of `values <ControlSignal.value>` of `control_signals <ControlMechanism.control_signals>`.
    FF
        Interaction among `state_feature_values <OptimizationControlMechanism_Feature_values>`.
    CC
        Interaction among `values <ControlSignal.value>` of `control_signals <ControlMechanism.control_signals>`.
    FC
        Interaction between `state_feature_values <OptimizationControlMechanism_Feature_values>` and
        `values <ControlSignal.value>` of `control_signals <ControlMechanism.control_signals>`.
    FFC
        Interaction between interactions of `state_feature_values
...
class PyTorchCFAError(CompositionFunctionApproximatorError):
    pass


class PyTorchCFA(CompositionFunctionApproximator):
    """
    PyTorchCFA(
    ...
    update_weights : LearningFunction, function or method : default BayesGLM
        parameterizes the `regression_weights <PyTorchCFA.regression_weights>` used by the `evaluate
        <PyTorchCFA.evaluate>` method to improve its prediction of `net_outcome <ControlMechanism.net_outcome>`
        ...
        <PyTorchCFA.prediction_vector>`, and the second item a 1d array containing a scalar
    ...
    prediction_terms : List[PV] : default [PV.F, PV.C, PV.COST]
        ...
        <PredictionVector.prediction_vector>` attribute of the `prediction_vector <PyTorchCFA.prediction_vector>`;
    ...
    update_weights : LearningFunction, function or method
        parameterizes the `regression_weights <PyTorchCFA.regression_weights>` used by the `evaluate
        <PyTorchCFA.evaluate>` method to improve prediction of `net_outcome <ControlMechanism.net_outcome>`
        from a given set of `state_feature_values <OptimizationControlMechanism.state_feature_values>` and a
        `control_allocation <ControlMechanism.control_allocation>` provided by an `OptimizationControlMechanism`;
        its result is assigned as the value of the `regression_weights <PyTorchCFA.regression_weights>` attribute.

    prediction_terms : List[PV]
        terms included in `vector <PredictionVector.prediction_vector>` attribute of the
        `prediction_vector <PyTorchCFA.prediction_vector>`;  items are members of the `PV` enum; the
    ...
    prediction_vector : PredictionVector
        ...
        `evaluate <PyTorchCFA.evaluate>`, along with `regression_weights <PyTorchCFA.regression_weights>` to
        make its prediction.  The values contained in the `vector <PredictionVector.vector>` attribute are
        determined by `prediction_terms <PyTorchCFA.prediction_terms>`.

    regression_weights : 1d array
        result returned by `update_weights <PyTorchCFA.update_weights>, and used by
        `evaluate <PyTorchCFA.evaluate>` method together with `prediction_vector <PyTorchCFA.prediction_vector>`
    ...
    class Parameters(CompositionFunctionApproximator.Parameters):
        """
            ...
            ----------
                ...
                prediction_vector
                    see `prediction_vector <PyTorchCFA.prediction_vector>`

                    :default value: None
                    :type:

                previous_state
                    see `previous_state <PyTorchCFA.previous_state>`

                    :default value: None
                    :type:

                regression_weights
                    see `regression_weights <PyTorchCFA.regression_weights>`

                    :default value: None
                    :type:

                update_weights
                    see `update_weights <PyTorchCFA.update_weights>`
    ...
    def _instantiate_prediction_terms(self, prediction_terms):
        ...
        # MODIFIED 11/9/18 END
            for term in self.prediction_terms:
                if term not in PV:
                    raise PyTorchCFAError("{} specified in {} arg of {} is not a member of the {} enum".
    ...
    def initialize(self, features_array, control_signals, context):
        """Assign owner and instantiate `prediction_vector <PyTorchCFA.prediction_vector>`

        Must be called before PyTorchCFA's methods can be used.
        """
    ...
    def adapt(self, feature_values, control_allocation, net_outcome, context=None):
        """Update `regression_weights <PyTorchCFA.regression_weights>` so as to improve prediction of
    ...
    # FIX: 11/3/21 - IMPLEMENT USE OF num_trials_per_estimate?
    ...
    def evaluate(self, feature_values, control_allocation, num_estimates, num_trials_per_estimate, context):
        """Update prediction_vector <PyTorchCFA.prediction_vector>`,
        then multiply by regression_weights.

        Uses the current values of `regression_weights <PyTorchCFA.regression_weights>` together with
        values of **state_feature_values** and **control_allocation** arguments to generate predicted `net_outcome
    ...
    class PredictionVector():
        ...
        def __init__(self, feature_values, control_signals, specified_terms):
            ...
            for c in control_signals:
                if isinstance(c, ControlSignal):
                    ...
                    port_spec_dict = _parse_port_spec(port_type=ControlSignal, owner=self, port_spec=c,
                                                      context=Context(string='PyTorchCFA.__init__'))
            ...
            def error_for_too_few_terms(term):
                ...
                raise PyTorchCFAError(f"Specification of {'PV.' + term} for {repr(PREDICTION_TERMS)} arg of "
            ...
            self.num[F] = len(f)  # state_feature_values are arrays
