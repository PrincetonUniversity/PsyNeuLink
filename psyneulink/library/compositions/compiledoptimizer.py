...
class Optimizer():
    ...
    def _get_delta_w_struct_type(self, ctx):
        delta_w = [None] * len(self._pytorch_model._projection_wrappers)
        for idx, proj in enumerate(self._pytorch_model._projection_wrappers):
    ...
    def step(self, ctx):
        ...
        for idx, proj in enumerate(self._pytorch_model._projection_wrappers):
            ...
            pnlvm.helpers.printf_float_matrix(ctx,
                                              builder,
                                              m_t_ptr,
                                              prefix=f"mt val: {proj.sender.mechanism} -> {proj.receiver.mechanism}\n",
                                              tags={"torch"})
        ...
        for idx, proj in enumerate(self._pytorch_model._projection_wrappers):
            ...
            pnlvm.helpers.printf_float_matrix(ctx,
                                              builder,
                                              delta_w_ptr,
                                              prefix=f"grad val: {proj.sender.mechanism} -> {proj.receiver.mechanism}\n",
                                              tags={"torch"})
