...
class Optimizer():
    ...
    def _get_param_gradients_struct_type(self, ctx):
        gradients_struct = [None] * len(self._pytorch_model.components)
        for idx, component in enumerate(self._pytorch_model.components):
            gradients_struct[idx] = component._get_learnable_param_struct_type(ctx)

        gradients_struct = pnlvm.ir.types.LiteralStructType(gradients_struct)
...
class AdamOptimizer(Optimizer):
    ...
    def _get_optimizer_struct_type(self, ctx):
        ...
        v_t = self._get_param_gradients_struct_type(ctx)  # current timestep second moment
