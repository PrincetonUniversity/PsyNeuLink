...
#        sorted_execution_list.sort(key=lambda object_item: object_item.phase)
    ...
    def show_graph(self,
        ...
        **show_mechanism_structure** is `True`, Mechanism nodes are subdivided into sections for its States with
        ...
        The figures below use the following example, which constructs a System from two Processes that converge
        on the same `TERMINAL` Mechanism::

            import psyneulink as pnl
            mech_1 = pnl.TransferMechanism(name='Mech 1`, size=3)
            mech_2 = pnl.TransferMechanism(name='Mech 2`, size=5)
            mech_3 = pnl.TransferMechanism(name='Mech 3`, size=2)
            my_process_A = pnl.Process(pathway=[mech_1, mech_3], learning=ENABLED)
            my_process_B = pnl.Process(pathway=[mech_2, mech_3])
            my_system = pnl.System(processes=[my_process_A, my_process_B]

        The figure below shows an example of the simplest form of the graph, generated by call the
        ``my_system.show_graph()`` with no parameters:


        The following shows examples using various options (see arguments, below):

        A. `my_system.show_graph(show_dimensions=True=True, show projection_labels=True)`
        B. `my_system.show_graph(show_learning=True)`
        C. `my_system.show_graph(show_mechanism_structure=True)`
        D. `my_system.show_graph(show_mechanism_structure=pnl.ALL)`

        The following figure shows the graph with the dimensions of its Components displayed, as well as
        the labels for its Projections, generated by calling it with ``my_system.show_graph(
        show_dimensions=True=True, show projection_labels=True)``:

        The following figure shows the graph with learning components, generated by calling
        ``my_system.show_graph(show_learning=True)``:

        This version uses the show_mechanism_structure option to show a more detailed display each of the
        System's Components, using ``my_system.show_graph(show_mechanism_structure=True)``:

        This version adds all of the option associated with show_mechanism_structure (see arguments below)
        `function <Component.function>`\s and `values <Component.value>`s:

        using ``my_system.show_graph(show_mechanism_structure=True XXXXXXX)``:
